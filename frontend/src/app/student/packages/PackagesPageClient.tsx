...
const ACCESS_TABS: {
  ...
  label: string;
  icon: LucideIcon;
  accessKind?: StudentQuickAccessKind;
}[] = [
  { key: 'packages', label: 'باقاتي', icon: Layers },
  { key: 'terms', label: 'ترماتي', icon: GraduationCap, accessKind: 'term' },
  { key: 'months', label: 'أقسامي', icon: CalendarDays, accessKind: 'section' },
  { key: 'lessons', label: 'حصصي', icon: BookOpen, accessKind: 'lesson' },
  {
    key: 'videos',
    label: 'فيديوهاتي',
    icon: Clapperboard,
...
export default function PackagesPageClient() {
      ...
      <div className="space-y-6 animate-pulse">
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          {[1, 2].map((i) => (
            <div
              key={i}
              className="h-80 rounded-2xl bg-[var(--admin-card-strong)]"
            />
          ))}
        </div>
      </div>
  ...
  if (
    hasLoadError &&
    packagesQuery.data === undefined &&
    quickAccessQuery.data === undefined
  ) {
    return (
      <div
        role="alert"
        className="rounded-2xl border border-[var(--admin-danger-20)] bg-[var(--admin-danger-10)] p-6 text-center"
      >
        <p className="font-bold text-[var(--admin-danger)]">
          تعذر تحميل الباقات حاليًا.
        </p>
        <button
          type="button"
          onClick={refetch}
          className="mt-4 min-h-11 rounded-xl bg-[var(--admin-primary)] px-5 text-sm font-black text-[var(--admin-primary-contrast)]"
        >
          إعادة المحاولة
        </button>
      </div>
  ...
  const directPackages = packages.filter(
    (pkg) => pkg.hasDirectPackageAccess ?? pkg.isEnrolled
  );
  const packagesNeedingActivation = packages.filter(
    (pkg) => !(pkg.hasDirectPackageAccess ?? pkg.isEnrolled)
  );
  ...
  const activeTabMeta =
    ACCESS_TABS.find((tab) => tab.key === activeTab) ?? ACCESS_TABS[0];
  const activeAccessKind = activeTabMeta.accessKind;
  const activeItems = activeAccessKind
    ? filterStudentQuickAccessItems(quickAccess, activeAccessKind)
    : [];
  ...
  return (
    <div className="space-y-12 pb-10">
      {hasLoadError && (
        <div
          role="alert"
          className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[var(--admin-warning-20)] bg-[var(--admin-warning-10)] px-4 py-3 text-sm font-bold text-[var(--admin-warning)]"
        >
      ...
      <section className="space-y-6">
        <div className="flex gap-2 overflow-x-auto rounded-2xl border border-[var(--admin-border)] bg-[var(--admin-card)] p-2">
          {ACCESS_TABS.map((tab) => {
            ...
            const tabAccessKind = tab.accessKind;
            const count = tabAccessKind
              ? filterStudentQuickAccessItems(quickAccess, tabAccessKind).length
            ...
            return (
              ...
              >
                ...
                <span>{tab.label}</span>
                <span
                  className={`rounded-full px-2 py-0.5 text-xs ${isActive ? 'bg-white/15' : 'bg-[var(--admin-card-soft)]'}`}
                >
                  {count}
                </span>
              </button>
  ...
  if (items.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-[var(--admin-border)] bg-[var(--admin-card)] p-10 text-center">
        ...
        <p className="mx-auto mt-2 max-w-lg text-sm font-medium leading-7 text-[var(--admin-muted)]">
          {emptyDescription}
        </p>
      </div>
    );
    ...
    <div className="space-y-4">
      <div className="border-b border-[var(--admin-border)] pb-4">
        ...
        <p className="mt-1 text-sm font-medium text-[var(--admin-muted)]">
          اختار أي عنصر وسنفتح لك مكانه داخل المسار مباشرة.
        </p>
      </div>
          ...
          >
            <div className="flex min-h-36 gap-4 p-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                <h3 className="mt-3 line-clamp-2 text-base font-black leading-6 text-[var(--admin-text)]">
                  {item.title}
                </h3>
                <p className="mt-2 line-clamp-2 text-xs font-medium leading-5 text-[var(--admin-muted)]">
                  {item.pathBreadcrumb}
                </p>
                {item.teacherName ? (
                  <div className="mt-3 flex items-center gap-2 text-xs font-bold text-[var(--admin-text)]">
                    {item.teacherProfileImageUrl ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img
                        src={resolveMediaUrl(item.teacherProfileImageUrl)}
                        alt={`صورة ${item.teacherName}`}
