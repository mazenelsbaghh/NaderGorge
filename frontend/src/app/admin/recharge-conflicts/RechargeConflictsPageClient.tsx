  ...
  return <div className="space-y-4" dir="rtl">
    ...
    {items.map(conflict => <section key={conflict.rechargeRequestId} className="admin-panel rounded-2xl p-5">
      ...
      <div className="grid gap-3 xl:grid-cols-2">{conflict.candidates.map(sms => <article key={sms.smsLogId} className="rounded-xl border border-[var(--admin-border)] p-4">
        ...
        <bdi className="mt-2 block font-mono text-sm">المحول: {sms.senderPhoneNumber}</bdi><bdi className="block font-mono text-xs">رقم العملية: {sms.transferReference || 'غير مستخرج'}</bdi><span className="block text-xs text-[var(--admin-muted)]">{new Date(sms.receivedAt).toLocaleString('ar-EG')}</span>
    ...
    {selected ? <div className="fixed inset-0 z-[100] grid place-items-center bg-black/50 p-4"><div className="w-full max-w-lg rounded-2xl bg-[var(--admin-card)] p-5 shadow-2xl"><h2 className="text-lg font-black">تأكيد نقل ربط التحويل</h2><p className="mt-2 text-sm">من <b>{selected.sms.matchedStudentName}</b> ({selected.sms.matchedStudentPhoneNumber}) إلى <b>{selected.conflict.studentName}</b> ({selected.conflict.studentPhoneNumber}). سيتم عكس الرصيد والقيد القديم أولًا ثم شحن الحساب الصحيح داخل معاملة واحدة.</p><textarea className="admin-input mt-4 min-h-24" value={reason} onChange={event => setReason(event.target.value)} placeholder="سبب النقل ومرجع المراجعة (مطلوب)" /><div className="mt-4 flex gap-2"><button className="admin-btn-primary" disabled={saving || !reason.trim()} onClick={() => void reassign()}>تأكيد النقل الآمن</button><button className="admin-btn-ghost" disabled={saving} onClick={() => { setSelected(null); setReason(''); }}>إلغاء</button></div></div></div> : null}
  </div>;
}
