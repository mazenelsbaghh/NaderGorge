  ...
  return <div className="space-y-4" dir="rtl">
    ...
    {items.map(conflict => <section key={conflict.rechargeRequestId} className="admin-panel rounded-2xl p-5">
      ...
      <div className="grid gap-3 xl:grid-cols-2">{conflict.candidates.map(sms => <article key={sms.smsLogId} className="rounded-xl border border-[var(--admin-border)] p-4">
        ...
        <bdi className="mt-2 block font-mono text-sm">المحول: {sms.senderPhoneNumber}</bdi><bdi className="block font-mono text-xs">رقم العملية: {sms.transferReference || 'غير مستخرج'}</bdi><span className="block text-xs text-[var(--admin-muted)]">{new Date(sms.receivedAt).toLocaleString('ar-EG-u-nu-latn', { timeZone: 'Africa/Cairo' })}</span>
