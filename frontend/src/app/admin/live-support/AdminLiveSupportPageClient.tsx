...
export default function AdminLiveSupportPageClient() {
  ...
  return <AdminShellChrome activePath="/admin/live-support" sectionLabel="خدمة العملاء" pageTitle="إدارة الدعم المباشر" subtitle="تفعيل الخدمة، تحديد السعة لكل موظف، وجدول المواعيد الذي يظهر للزوار خارج أوقات الدعم.">
    ...
    {!config || !dashboard ? <div className="grid min-h-80 place-items-center"><LoaderCircle className="animate-spin"/></div> : <div dir="rtl" className="space-y-5">
      ...
      <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white"><div className="flex flex-wrap items-end justify-between gap-3 border-b border-slate-100 p-5"><div><h2 className="font-bold text-slate-900">كل المحادثات والنشاط</h2><p className="mt-1 text-sm text-slate-500">تتحدث القائمة تلقائيًا كل 10 ثوانٍ، وتشمل حالات AI والـWorker الآمنة.</p></div><label className="text-sm font-semibold text-slate-700">تصفية النشاط<select value={conversationFilter} onChange={event => setConversationFilter(event.target.value as typeof conversationFilter)} className="mr-2 h-11 rounded-xl border border-slate-200 px-3"><option value="all">الكل</option><option value="ai">نشاط AI</option><option value="failed">فشل AI / Worker</option></select></label></div><div className="overflow-x-auto"><table className="w-full min-w-[980px] text-right text-sm"><thead className="bg-slate-50 text-xs text-slate-500"><tr><th className="p-3">الشخص</th><th className="p-3">الحالة</th><th className="p-3">AI / Worker</th><th className="p-3">الموظف</th><th className="p-3">وقت البدء</th><th className="p-3">الانتظار</th><th className="p-3">مدة التعامل</th><th className="p-3">المتابعة</th></tr></thead><tbody>{dashboard.conversations.filter(item => conversationFilter === 'all' || (conversationFilter === 'ai' ? Boolean(item.aiTurnStatus) : item.aiTurnStatus === 'Failed')).map((item) => <tr key={item.id} className="border-t border-slate-100 hover:bg-cyan-50"><td className="p-3 font-semibold text-slate-900">{item.participantName}<span className="mr-2 text-xs font-normal text-slate-500">{item.participantType === 'Guest' ? 'زائر' : 'طالب'}</span></td><td className="p-3">{item.status}</td><td className="p-3"><span className="font-semibold">{item.aiTurnStatus || 'بشري'}</span>{item.aiTurnFailureCode && <bdi dir="ltr" className="mt-1 block break-all text-xs text-red-700">{item.aiTurnFailureCode}</bdi>}</td><td className="p-3">{item.ownerName || 'الطابور'}</td><td className="p-3">{new Date(item.createdAt).toLocaleString('ar-EG')}</td><td className="p-3">{formatDuration(item.waitSeconds)}</td><td className="p-3">{formatDuration(item.handleSeconds)}</td><td className="p-3"><button type="button" onClick={() => void liveSupportService.getAdminTimeline(item.id).then(setTimeline)} className="min-h-10 rounded-lg bg-slate-900 px-3 font-semibold text-white">فتح المحادثة</button></td></tr>)}</tbody></table></div></section>
