...
export default function AdminLiveSupportPageClient() {
  ...
  return <AdminShellChrome activePath="/admin/live-support" sectionLabel="خدمة العملاء" pageTitle="إدارة الدعم المباشر" subtitle="تفعيل الخدمة، تحديد السعة لكل موظف، وجدول المواعيد الذي يظهر للزوار خارج أوقات الدعم.">
    ...
    {!config || !dashboard ? <SupportPageSkeleton /> : <div dir="rtl" className="space-y-5">
      ...
      <StaffConfigurationPanel><p className="rounded-xl bg-[var(--admin-warning-10)] p-4 text-sm text-[var(--admin-text)]">صلاحية «إدارة الدعم المباشر» وحدها لا تجعل الموظف يستقبل محادثات. فعّل الموظف هنا وحدد سعته ثم احفظ.</p>{config.staff.map((staff) => <StaffCard key={staff.userId} staff={staff} isDirty={serializeStaff(staff) !== staffBaselines[staff.userId]} feedback={staffFeedback[staff.userId]} isSaving={savingStaffId === staff.userId} update={(change) => updateStaff(staff.userId, change)} save={() => void saveStaff(staff)}/>)}</StaffConfigurationPanel>
    </div>}
