...
function StaffCard({ staff, isDirty, feedback, isSaving, update, save }: { staff: LiveSupportStaffConfig; isDirty: boolean; feedback?: string; isSaving: boolean; update: (change: Partial<LiveSupportStaffConfig>) => void; save: () => void }) {
  ...
  return <article className="rounded-2xl border border-[var(--admin-border)] bg-[var(--admin-card)] p-5 shadow-[var(--admin-shadow)]">
    ...
    <div className="mt-4 grid gap-4 lg:grid-cols-[220px_1fr_auto]">
      ...
      <div><div className="mb-2 flex items-center justify-between"><span className="flex items-center gap-2 text-sm font-medium text-[var(--admin-text)]"><Clock3 size={16}/>مواعيد الدعم</span><button type="button" onClick={() => update({ schedule: [...staff.schedule, { dayOfWeek: 0, startLocalTime: '09:00:00', endLocalTime: '17:00:00' }] })} className="min-h-11 text-sm font-semibold text-[var(--admin-accent)] transition hover:text-[var(--admin-primary)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--admin-accent)]">+ إضافة موعد</button></div><p className="mb-2 text-xs font-medium text-[var(--admin-muted)]">اكتب وقت البداية ثم النهاية؛ لو النهاية بعد منتصف الليل تُحسب تلقائيًا في اليوم التالي.</p><div className="space-y-3">{staff.schedule.map((window, index) => <div key={`${index}-${window.dayOfWeek}`} className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_2.75rem] items-end gap-2 rounded-xl bg-[var(--admin-card-soft)] p-2 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_2.75rem]"><label className="col-span-3 min-w-0 text-xs font-bold text-[var(--admin-muted)] sm:col-span-1">اليوم<select value={window.dayOfWeek} onChange={(event) => updateWindow(index, { dayOfWeek: Number(event.target.value) })} className="mt-1 h-11 w-full min-w-0 rounded-lg border border-[var(--admin-border)] bg-[var(--admin-card)] px-2 text-[var(--admin-text)]"><option value="">اليوم</option>{days.map((day, value) => <option key={day} value={value}>{day}</option>)}</select></label><label className="min-w-0 text-xs font-bold text-[var(--admin-muted)]">من<input aria-label="وقت بداية الدعم" dir="ltr" type="time" value={window.startLocalTime.slice(0,5)} onChange={(event) => updateWindow(index, { startLocalTime: `${event.target.value}:00` })} className="mt-1 h-11 w-full min-w-0 rounded-lg border border-[var(--admin-border)] bg-[var(--admin-card)] px-2 text-[var(--admin-text)]"/></label><label className="min-w-0 text-xs font-bold text-[var(--admin-muted)]">إلى<input aria-label="وقت نهاية الدعم" dir="ltr" type="time" value={window.endLocalTime.slice(0,5)} onChange={(event) => updateWindow(index, { endLocalTime: `${event.target.value}:00` })} className="mt-1 h-11 w-full min-w-0 rounded-lg border border-[var(--admin-border)] bg-[var(--admin-card)] px-2 text-[var(--admin-text)]"/></label><button type="button" aria-label="حذف الموعد" onClick={() => update({ schedule: staff.schedule.filter((_, current) => current !== index) })} className="size-11 rounded-lg text-[var(--admin-danger)] transition hover:bg-[var(--admin-danger-10)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--admin-danger)]">×</button></div>)}</div></div>
