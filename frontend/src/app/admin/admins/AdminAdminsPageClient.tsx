...
export default function AdminAdminsPageClient() {
  ...
  const [selectedAdmin, setSelectedAdmin] = useState<AdminUserListDto | null>(null);
  ...
  async function handleToggleStatus(user: AdminUserListDto) {
    if (currentUser?.id === user.id) {
      toast.error('لا يمكنك تعليق حسابك الشخصي!');
      return;
    }
    ...
    try {
      ...
      setConfirmUser(null);
      toast.success(
        nextStatus === 'Active' ? 'تم تنشيط حساب المدير' : 'تم تعليق حساب المدير'
      );
    } catch (error) {
      devConsole.error(error);
      toast.error('حدث خطأ أثناء تحديث حالة المدير، أعد المحاولة.');
    }
  }
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      link.setAttribute(
        'download',
        `قائمة_المديرين_${new Date().toISOString().split('T')[0]}.csv`
      );
  ...
  const filteredAdmins = users.filter((user) => normalizeRole(user) === 'Admin');
  const activeCount = filteredAdmins.filter((user) => user.status === 'Active').length;
  const pendingCount = filteredAdmins.filter((user) => user.status !== 'Active').length;
  ...
  return (
    <AdminShellChrome
      ...
      subtitle="إدارة الحسابات الإدارية، تعيين الأدوار والصلاحيات، ومراقبة الحالات ونشاطات المديرين."
      action={
        <NeumorphButton
          intent="primary"
          size="lg"
    ...
    >
      ...
      <ConfirmDialog
        ...
        description={`هل أنت متأكد من تغيير حالة حساب المدير "${confirmUser?.fullName}"؟`}
        confirmLabel={
          confirmUser?.status === 'Active'
            ? 'نعم، تعليق الحساب'
            : 'نعم، تنشيط الحساب'
      ...
      {loadError ? (
        <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-[var(--admin-border)] p-16 text-center gap-6 bg-[var(--admin-card-soft)]">
          ...
          <h4 className="text-xl font-black text-[var(--admin-text)]">تعذّر تحميل قائمة المديرين</h4>
        ...
        <>
          <section className="mb-12 grid grid-cols-1 gap-6 md:grid-cols-3">
            <AdminStatCard
              ...
              subtitle="إجمالي المديرين المسجلين"
            />
            <AdminStatCard
              variant="accent"
              icon={Sparkles}
          ...
          <AdminDataTable
            ...
            loading={loading}
            rowKey={(u) => u.id}
            emptyMessage="لا توجد نتائج مطابقة لفلترة المديرين."
            onRowClick={(u) => {
              setSelectedAdmin(u);
            }}
          />
