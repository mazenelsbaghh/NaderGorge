  ...
  return <AdminPage activePath="/admin/wallet-messages" sectionLabel="المالية" pageTitle="رسائل المحافظ" subtitle="كل رسائل المحافظ مع البحث وحالة المطابقة بطلبات الشحن.">
    <div className="space-y-5" dir="rtl">
      ...
      <section className="admin-panel rounded-2xl p-5">
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-sm"><thead><tr className="text-right"><th>التاريخ</th><th>المحفظة</th><th>رقم المحول</th><th>المبلغ</th><th>الرسالة</th><th>المطابقة</th><th>الطالب المرتبط</th></tr></thead><tbody>{messages.map(message => <tr key={message.id} className="border-t border-[var(--admin-border)] align-top"><td className="whitespace-nowrap">{new Date(message.receivedAt).toLocaleString('ar-EG', { dateStyle: 'medium', timeStyle: 'short' })}</td><td><b>{message.walletLabel}</b><bdi className="block font-mono text-xs text-[var(--admin-muted)]">{message.walletPhoneNumber}</bdi></td><td><bdi className="font-mono font-bold">{message.parsedSenderPhone || '—'}</bdi></td><td className="whitespace-nowrap font-mono font-bold">{message.parsedAmount != null ? `${message.parsedAmount} ج.م` : '—'}</td><td><p className="max-w-xl whitespace-pre-wrap leading-6">{message.body}</p></td><td>{message.isMatched ? <span className="inline-flex items-center gap-1 font-bold text-emerald-600"><CheckCircle2 className="h-4 w-4" /> مطابقة</span> : <span className="inline-flex items-center gap-1 font-bold text-rose-600"><XCircle className="h-4 w-4" /> غير مطابقة</span>}</td><td>{message.matchedRechargeRequestId ? <div>{message.matchedStudentName ? <Link className="font-bold text-[var(--admin-primary)] hover:underline" href={`/admin/recharge-verification?search=${encodeURIComponent(message.matchedStudentPhoneNumber || '')}`}>{message.matchedStudentName}</Link> : 'طلب شحن مرتبط'}<bdi className="block font-mono text-xs text-[var(--admin-muted)]">{message.matchedStudentPhoneNumber}</bdi></div> : '—'}</td></tr>)}</tbody></table>{!loading && messages.length === 0 ? <p className="py-10 text-center text-[var(--admin-muted)]">لا توجد رسائل بهذه المواصفات.</p> : null}{loading ? <p className="py-10 text-center text-[var(--admin-muted)]">جارٍ تحميل الرسائل…</p> : null}</div>
