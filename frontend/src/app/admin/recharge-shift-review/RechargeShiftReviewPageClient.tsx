...
export function RechargeShiftReviewWorkspace() {
  ...
  function exportExcel() {
    ...
    const rows = report.items.map(item => [item.studentName, item.studentPhoneNumber, item.acceptanceMethod, item.amount, item.balanceScope, item.teacherName, item.balanceBefore, item.balanceAfter, item.currentBalance, `${item.walletLabel} ${item.walletPhoneNumber}`, item.senderPhoneNumber, item.resolvedByUserName, new Date(item.resolvedAt).toLocaleString('ar-EG'), item.duplicateReason || 'لا', item.isReversed ? 'تم العكس' : item.canReverse ? 'متاح' : item.reverseBlockedReason]);
  ...
  return <div className="space-y-5" dir="rtl">
    ...
    <section className="admin-panel rounded-2xl p-5"><div className="overflow-x-auto"><table className="w-full min-w-[1300px] text-sm"><thead><tr className="text-right"><th>الطالب</th><th>القبول</th><th>المبلغ والرصيد</th><th>قبل / بعد / حالي</th><th>المحفظة والمحول</th><th>المسؤول والوقت</th><th>مراجعة التكرار</th><th>الإجراء</th></tr></thead><tbody>{report?.items.map(item => <tr key={item.rechargeRequestId} className="border-t border-[var(--admin-border)] align-top"><td><b>{item.studentName}</b><bdi className="block font-mono">{item.studentPhoneNumber}</bdi></td><td><span className={item.acceptanceMethod === 'آلي' ? 'font-bold text-emerald-600' : 'font-bold text-sky-700'}>{item.acceptanceMethod}</span></td><td><b>{item.amount.toLocaleString('ar-EG')} ج.م</b><span className="block text-xs text-[var(--admin-muted)]">{item.balanceScope}{item.teacherName ? ` — ${item.teacherName}` : ''}</span></td><td className="font-mono"><span>{item.balanceBefore ?? '—'}</span> / <span>{item.balanceAfter ?? '—'}</span> / <b>{item.currentBalance}</b></td><td><b>{item.walletLabel}</b><bdi className="block font-mono text-xs">من: {item.senderPhoneNumber || '—'}</bdi></td><td><b>{item.resolvedByUserName}</b><span className="block text-xs">{new Date(item.resolvedAt).toLocaleString('ar-EG')}</span></td><td>{item.suspectedDuplicate ? <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-2 py-1 font-bold text-amber-800"><AlertTriangle className="h-4 w-4" /> مشتبه</span> : <span className="text-[var(--admin-muted)]">لا يوجد</span>}</td><td>{item.isReversed ? <b className="text-rose-600">تم العكس</b> : <button type="button" className="admin-btn-ghost" disabled={!item.canReverse} title={item.reverseBlockedReason} onClick={() => setSelected(item)}><RotateCcw className="h-4 w-4" /> عكس آمن</button>}{!item.canReverse && !item.isReversed ? <small className="mt-1 block max-w-48 text-amber-700">{item.reverseBlockedReason}</small> : null}</td></tr>)}</tbody></table>{loading ? <p className="py-10 text-center">جارٍ التحميل…</p> : !report?.items.length ? <p className="py-10 text-center text-[var(--admin-muted)]">لا توجد عمليات مقبولة في الفترة.</p> : null}</div></section>
    {selected ? <div className="fixed inset-0 z-[100] grid place-items-center bg-slate-950/55 p-4"><div className="admin-panel w-full max-w-lg rounded-2xl p-6"><h2 className="text-xl font-black">تأكيد عكس الشحن</h2><p className="mt-2 text-sm">سيُخصم {selected.amount} ج.م من رصيد الطالب والمحفظة ويُعكس القيد المالي. العملية مسجلة باسمك.</p><textarea className="admin-input mt-4 min-h-28" placeholder="اكتب سبب العكس المالي (إجباري)" value={reason} onChange={event => setReason(event.target.value)} /><div className="mt-4 flex gap-2"><button className="admin-btn-primary" type="button" disabled={!reason.trim() || reversing} onClick={() => void reverseCredit()}>{reversing ? 'جارٍ العكس…' : 'تأكيد العكس'}</button><button className="admin-btn-ghost" type="button" disabled={reversing} onClick={() => { setSelected(undefined); setReason(''); }}>إلغاء</button></div></div></div> : null}
  </div>;
}
