...
export default function HrShiftsPageClient() {
  ...
  async function publishWeeklySchedule(event: FormEvent) {
    ...
    try {
      ...
      if (!existingSchedule.valid) return setConflict('يوجد شفت منشور للموظف في هذه الفترة. أنهِ التعيين السابق أولاً أو اختر تاريخاً آخر.');
  ...
  return <AdminPage activePath="/admin/hr/shifts" sectionLabel="الموارد البشرية" pageTitle="تخطيط الشفتات" subtitle="قوالب ثابتة ومرنة وليلية ومتعددة الفترات مع كشف التعارض قبل النشر." action={<button type="button" onClick={() => void load()} className="admin-btn-primary inline-flex min-h-11 items-center gap-2"><RefreshCw className="h-4 w-4" />تحديث</button>}>
    {loading ? <AdminPageSkeleton /> : <div className="space-y-6">
      ...
      <section className="space-y-3"><div><h2 className="text-lg font-black">تقويمات العمل وأيام الراحة</h2><p className="mt-1 text-sm text-[var(--admin-muted)]">حدد أيام العمل لكل تقويم؛ الأيام غير المحددة تُحسب راحة أسبوعية لكل الشيفتات المرتبطة به.</p></div><div className="grid gap-3 lg:grid-cols-2">{calendars.map((calendar) => <article key={calendar.id} className="admin-panel"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-black">{calendar.name}</p><p className="text-xs font-bold text-[var(--admin-muted)]">{calendar.timeZoneId}</p></div><button type="button" onClick={() => void saveCalendar(calendar.id)} className="admin-btn-primary min-h-10">حفظ الأيام</button></div><div className="mt-4 grid grid-cols-4 gap-2 sm:grid-cols-7">{weekDays.map((day, dayOfWeek) => { const isWorking = ((calendarDrafts[calendar.id] ?? 0) & (1 << dayOfWeek)) !== 0; return <label key={day} className={`flex min-h-16 cursor-pointer flex-col items-center justify-center rounded-xl border px-2 text-xs font-bold ${isWorking ? 'border-[var(--admin-primary)] bg-[var(--admin-primary-15)] text-[var(--admin-primary)]' : 'border-[var(--admin-border)] bg-[var(--admin-bg)] text-[var(--admin-muted)]'}`}><input type="checkbox" checked={isWorking} onChange={() => toggleCalendarDay(calendar.id, dayOfWeek)} className="sr-only" /><span>{day}</span><span className="mt-1 text-[10px]">{isWorking ? 'عمل' : 'راحة'}</span></label>; })}</div></article>)}</div></section>
