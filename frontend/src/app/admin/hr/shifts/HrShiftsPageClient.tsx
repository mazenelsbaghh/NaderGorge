...
export default function HrShiftsPageClient() {
  ...
  const [templateForm, setTemplateForm] = useState({ code: '', name: '', startsAt: '09:00', endsAt: '17:00', mode: 'Fixed', workCalendarId: '', breakMinutes: 30, overtimeAfterMinutes: 480 });
  ...
  async function createTemplate(event: FormEvent) {
    ...
    try { await hrService.createShiftTemplate({ code: templateForm.code, name: templateForm.name, mode: templateForm.mode, workCalendarId: templateForm.workCalendarId, minimumBreakMinutes: templateForm.breakMinutes, overtimeAfterMinutes: templateForm.overtimeAfterMinutes, segments: [{ sequence: 1, dayOfWeek: null, startsAt: `${templateForm.startsAt}:00`, endsAt: `${templateForm.endsAt}:00`, unpaidBreakMinutes: templateForm.breakMinutes, workDateRule: 'SegmentStartDate' }] }); toast.success('تم إنشاء قالب الشفت'); setTemplateForm((current) => ({ code: '', name: '', startsAt: '09:00', endsAt: '17:00', mode: 'Fixed', workCalendarId: current.workCalendarId, breakMinutes: 30, overtimeAfterMinutes: 480 })); await load(); }
  ...
  async function publishWeeklySchedule(event: FormEvent) {
    ...
    try {
      ...
      const created = await hrService.createShiftTemplate({ code: `WEEK-${employeeCode}-${dateCode}`, name: `جدول أسبوعي: ${employees.find((employee) => employee.id === weeklyForm.employeeId)?.fullName ?? ''}`, mode: 'Fixed', workCalendarId: calendarId, minimumBreakMinutes: 30, overtimeAfterMinutes: 480, segments });
  ...
  return <AdminPage activePath="/admin/hr/shifts" sectionLabel="الموارد البشرية" pageTitle="تخطيط الشفتات" subtitle="قوالب ثابتة ومرنة وليلية ومتعددة الفترات مع كشف التعارض قبل النشر." action={<button type="button" onClick={() => void load()} className="admin-btn-primary inline-flex min-h-11 items-center gap-2"><RefreshCw className="h-4 w-4" />تحديث</button>}>
    {loading ? <AdminPageSkeleton /> : <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-2"><form onSubmit={createTemplate} className="admin-panel space-y-4"><div className="flex items-center gap-2"><Plus className="h-5 w-5 text-[var(--admin-primary)]" /><h2 className="text-lg font-black">قالب شفت جديد</h2></div><div className="grid gap-3 sm:grid-cols-2"><label className="text-sm font-bold">الكود<input required value={templateForm.code} onChange={(e) => setTemplateForm((v) => ({ ...v, code: e.target.value }))} className="admin-input mt-1 w-full" /></label><label className="text-sm font-bold">الاسم<input required value={templateForm.name} onChange={(e) => setTemplateForm((v) => ({ ...v, name: e.target.value }))} className="admin-input mt-1 w-full" /></label><label className="text-sm font-bold">البداية<input aria-label="بداية الشفت" type="time" value={templateForm.startsAt} onChange={(e) => setTemplateForm((v) => ({ ...v, startsAt: e.target.value }))} className="admin-input mt-1 w-full" /></label><label className="text-sm font-bold">النهاية<input aria-label="نهاية الشفت" type="time" value={templateForm.endsAt} onChange={(e) => setTemplateForm((v) => ({ ...v, endsAt: e.target.value }))} className="admin-input mt-1 w-full" /></label><label className="text-sm font-bold">الراحة غير المدفوعة<input required min={0} type="number" value={templateForm.breakMinutes} onChange={(e) => setTemplateForm((v) => ({ ...v, breakMinutes: Number(e.target.value) }))} className="admin-input mt-1 w-full" /></label><label className="text-sm font-bold">بدء الإضافي بعد (دقيقة)<input required min={1} type="number" value={templateForm.overtimeAfterMinutes} onChange={(e) => setTemplateForm((v) => ({ ...v, overtimeAfterMinutes: Number(e.target.value) }))} className="admin-input mt-1 w-full" /></label><label className="text-sm font-bold">نوع الشفت<select value={templateForm.mode} onChange={(e) => setTemplateForm((v) => ({ ...v, mode: e.target.value }))} className="admin-input mt-1 w-full"><option value="Fixed">ثابت</option><option value="Flexible">مرن</option><option value="Split">متعدد الفترات</option></select></label><label className="text-sm font-bold sm:col-span-2">تقويم العمل وأيام الراحة<select required value={templateForm.workCalendarId} onChange={(e) => setTemplateForm((v) => ({ ...v, workCalendarId: e.target.value }))} className="admin-input mt-1 w-full"><option value="">اختر تقويم العمل</option>{calendars.map((calendar) => <option key={calendar.id} value={calendar.id}>{calendar.name}</option>)}</select></label></div>{selectedCalendar && <p className="rounded-xl bg-[var(--admin-bg)] p-3 text-xs font-bold text-[var(--admin-muted)]">أيام الراحة في هذا التقويم: <span className="text-[var(--admin-text)]">{selectedRestDays.length > 0 ? selectedRestDays.join('، ') : 'لا توجد'}</span></p>}<button className="admin-btn-primary min-h-11 w-full">حفظ القالب</button></form>
