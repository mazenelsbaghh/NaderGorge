...
function generateBunnyHlsEmbedHtml(signedPlaylistUrl: string, studentName: string, studentPhone: string): string {
  let parsedUrl: URL;
  try {
  ...
  if (parsedUrl.protocol !== 'https:' || !parsedUrl.hostname.endsWith('.b-cdn.net')) {
...
(function(){
  'use strict';
  var source=${safeSource}; var video=document.getElementById('video'); var hls=null; var readySent=false;
  var nativeLevels=[]; var nativeCurrent='auto'; var masterSource=source; var mediaRecoveries=0; var terminalErrorSent=false; var loadDeadline=null; var playbackDeadline=null; var lastLoadPhase='bootstrap'; var lastMediaTime=0;
  ...
  function armPlaybackDeadline(phase){clearPlaybackDeadline();lastLoadPhase=phase||lastLoadPhase;playbackDeadline=setTimeout(function(){if(!video.paused&&!video.ended)failHls(0,'بدأ تحميل Bunny HLS لكن لم تصل بيانات الفيديو للجهاز. راجع اتصال Bunny CDN أو جرّب شبكة أخرى.','playback_timeout_'+lastLoadPhase)},15000);}
  ...
  function attachEvents(){
    ...
    video.addEventListener('timeupdate',function(){var current=Number(video.currentTime)||0;if(current>lastMediaTime+.01){lastMediaTime=current;clearPlaybackDeadline();}post('timeUpdate',state());});
    video.addEventListener('play',function(){armPlaybackDeadline('play');post('stateChange',state());});
    video.addEventListener('pause',function(){post('stateChange',state());});
    video.addEventListener('ended',function(){post('stateChange',state());});
    ...
    video.addEventListener('error',function(){if(!terminalErrorSent)failHls(0);});
  }
  ...
  loadDeadline=setTimeout(function(){failHls(0,'انتهت مهلة تجهيز فيديو Bunny HLS قبل وصول بيانات التشغيل. راجع CDN Token Authentication Key وAllowed Domains ثم أعد المحاولة.','load_timeout_'+lastLoadPhase)},20000);
  if(window.Hls&&window.Hls.isSupported()){
    hls=new window.Hls({enableWorker:true,capLevelToPlayerSize:true,startLevel:-1,manifestLoadPolicy:noRetryLoadPolicy,playlistLoadPolicy:noRetryLoadPolicy,keyLoadPolicy:noRetryLoadPolicy,fragLoadPolicy:noRetryFragmentPolicy});hls.loadSource(source);hls.attachMedia(video);
    ...
    hls.on(window.Hls.Events.ERROR,function(_,data){if(!data||!data.fatal)return;if(data.type===window.Hls.ErrorTypes.MEDIA_ERROR&&mediaRecoveries<1){mediaRecoveries++;hls.recoverMediaError();return;}var status=data&&data.response?Number(data.response.code||data.response.status||0):0;var phase=data&&data.details?String(data.details):'network';failHls(status,hlsErrorMessage(status)+' ['+phase+']',phase);});
  }else if(video.canPlayType('application/vnd.apple.mpegurl')){
    fetch(source,{credentials:'omit',referrerPolicy:'strict-origin-when-cross-origin'}).then(function(r){if(!r.ok){failHls(r.status,undefined,'native_manifest_http');return '';}return r.text();}).then(function(text){if(terminalErrorSent||!text)return;lastLoadPhase='native_manifest_loaded';parseNativeMaster(text);video.src=source;video.load()}).catch(function(){failHls(0,undefined,'native_manifest_network')});
