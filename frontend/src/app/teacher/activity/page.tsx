...
import { Users, AlertTriangle, Play, Flame, Clock, Calendar, Sparkles } from "lucide-react";
        ...
        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full bg-[var(--admin-primary-15)] px-4 py-1 text-xs font-black text-[var(--admin-primary)]">
              ...
              نشاط الطلاب والتحليلات
            </div>
            <h1 className="mt-4 text-3xl font-black text-[var(--admin-text)] md:text-4xl">
              تتبع نشاط الطلاب
            </h1>
            <p className="mt-2 text-sm text-[var(--admin-muted)]">
              راقب تفاعل طلابك مع المحتوى التعليمي، واكتشف الفيديوهات الأكثر مشاهدة ومتابعة حالات الخمول.
            </p>
          </div>
        </div>
      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {data.activeStudents.map((student, idx) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 text-xs text-[var(--admin-text)] bg-[var(--admin-card)]/50 p-2.5 rounded-xl border border-[var(--admin-border)]">
