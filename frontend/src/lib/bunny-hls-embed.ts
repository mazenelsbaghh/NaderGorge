...
(function(){
  ...
  function failHls(status,message,phase){if(terminalErrorSent)return;terminalErrorSent=true;clearRenewalTimer();waitingLoaders.clear();if(loadDeadline)clearTimeout(loadDeadline);clearPlaybackDeadline();if(hls)try{hls.destroy()}catch(e){}video.pause();if(nativePlayback){video.removeAttribute('src');video.load();}post('error',{provider:'bunny-hls',code:Number(status)||0,phase:((relayAttempted?'relay_':'')+String(phase||'unknown')).slice(0,80),message:message||hlsErrorMessage(Number(status)||0)});}
