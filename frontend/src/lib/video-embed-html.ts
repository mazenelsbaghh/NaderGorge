...
export function generateVideoEmbedHtml(provider: string, videoId: string, options: {
  ...
  if (normalizedProvider === 'bunny') {
    return generateBunnyEmbedHtml(videoId, studentName, studentPhone, bunnyEmbedQuery);
  }
...
function generateBunnyEmbedHtml(videoId: string, studentName: string, studentPhone: string, signedQuery?: string): string {
  const reference = resolveBunnyVideoReference(videoId);
  if (typeof reference === 'string') return embedErrorHtml(reference);

  if (signedQuery && !/^token=[a-f0-9]{64}&expires=\d+$/.test(signedQuery)) return embedErrorHtml('تعذر تجهيز توقيع مشغّل الفيديو.');
  const playerQuery = 'autoplay=false&playsinline=true&disableIosPlayer=true' + (signedQuery ? '&' + signedQuery : '');
...
function generateYouTubeEmbedHtml(videoId: string, studentName: string, studentPhone: string, { qualityPreview = false, bottomCoverPercent = 0, mobileBottomCoverPercent = 0 }: { qualityPreview?: boolean; bottomCoverPercent?: number; mobileBottomCoverPercent?: number } = {}): string {
  ...
  const watermarkBrand = inlineScriptString('Massar Academy');
  const watermarkStudentName = inlineScriptString(studentName);
  const watermarkStudentPhone = inlineScriptString(studentPhone);
