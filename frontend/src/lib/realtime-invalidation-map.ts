...
import type { PlatformQueryKey } from '@/lib/query-keys';
...
const STUDENT_QUERY_PREFIXES: Record<string, readonly PlatformQueryKey[]> = {
  'student:shell': [['student', 'shell']],
  'student:dashboard': [['student', 'dashboard']],
  'student:quick-access': [['student', 'quick-access']],
  'student:exams': [['student', 'dashboard']],
  'student:homeworks': [['student', 'dashboard']],
  'student:balance': [['student', 'shell']],
  notifications: [['student', 'shell']],
  'content:packages': [
    ['student', 'packages'],
    ['student', 'teachers'],
    ['student', 'dashboard'],
...
export function invalidateStudentQueryKeys(keys: readonly string[]): void {
  const prefixes = new Map<string, PlatformQueryKey>();
  for (const key of keys) {
    for (const prefix of STUDENT_QUERY_PREFIXES[key] ?? []) {
      prefixes.set(JSON.stringify(prefix), prefix);
    }
  }
  for (const prefix of prefixes.values()) {
    platformQueryClient.invalidateQueries(prefix);
  }
}
...
export function invalidateForStaffDataChanged(payload: StaffDataChangedPayload): boolean {
  ...
  if (keys.size > 0) {
    ...
    invalidateMany([...keys]);
    invalidateStudentQueryKeys([...keys]);
  }
  return true;
}
