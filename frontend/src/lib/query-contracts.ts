...
export const ADMIN_AI_REFRESH_SCOPE_KEYS = {
  identity: ['employees', 'users', 'student:shell'],
  content: ['content:packages', 'content:lessons', 'assessments'],
  commercial: ['codes:groups', 'forms', 'reports'],
  finance: ['finance:payroll', 'finance:teacher', 'student:balance', 'reports'],
  hr: ['hr:employees', 'hr:organization', 'hr:payroll', 'hr:reports'],
  support: ['support:staff', 'support:dashboard', 'support:ai'],
  reporting: ['reports', 'media'],
  other: ['admin'],
} as const;

export type AdminAiRefreshScope = keyof typeof ADMIN_AI_REFRESH_SCOPE_KEYS;

export function adminAiRefreshKeys(scope: string): readonly string[] {
  if (!(scope in ADMIN_AI_REFRESH_SCOPE_KEYS)) {
    throw new Error(`Unknown AdminAI refresh scope: ${scope}`);
  }
  return ADMIN_AI_REFRESH_SCOPE_KEYS[scope as AdminAiRefreshScope];
}
...
export const queryContracts: readonly QueryContract[] = [
  ...
  },
  {
    owner: 'content',
    domain: 'content',
    scopes: ['content', 'subjects'],
    keys: ['content:packages', 'content:lessons'],
  ...
  },
  {
    owner: 'codes-finance',
    domain: 'codes',
    scopes: ['codes', 'finance', 'balance'],
    keys: ['codes:groups', 'finance:payroll', 'student:balance'],
    operations: ['create', 'update', 'delete', 'bulk'],
    strategy: 'invalidate',
  },
  {
    owner: 'assessment',
    domain: 'assessments',
    scopes: ['assessments', 'activity'],
    keys: ['student:exams', 'student:homeworks', 'assessments'],
    operations: ['create', 'update', 'delete'],
    strategy: 'invalidate',
  },
  {
    owner: 'community-notifications',
    domain: 'community',
    scopes: ['community', 'notifications', 'comments'],
    keys: ['community:posts', 'notifications', 'student:shell'],
    operations: ['create', 'update', 'delete'],
    strategy: 'invalidate',
  },
  {
    owner: 'operations-crm-support',
    domain: 'operations',
    scopes: ['operations', 'crm', 'support'],
    keys: ['operations:tasks', 'crm:queues', 'support:staff'],
    operations: ['create', 'update', 'delete', 'bulk'],
    strategy: 'invalidate',
  },
  {
    owner: 'forms-media-reports',
    domain: 'forms',
    scopes: ['forms', 'media', 'reports'],
    keys: ['forms', 'media', 'reports'],
    operations: ['create', 'update', 'delete', 'bulk'],
    strategy: 'invalidate',
  },
];
 ...
 */
type MutationContractSeed = readonly [
  string,
  number,
  DataDomain,
  readonly string[],
...
export const mutationContracts: readonly MutationContractSeed[] = [
  ...
  [
    ...
    5,
    'operations',
    ['operations:tasks', 'operations:dashboard'],
  ],
  ...
  [
    ...
    8,
    'finance',
    ['finance:payroll', 'finance:teacher', 'reports'],
  ],
  ...
  [
    ...
    4,
    'content',
    ['content:packages', 'student:shell'],
  ],
  [
    'student-service.ts',
    6,
    'community',
    ['community:posts', 'notifications', 'student:shell'],
  ],
  [
    'teacher-service.ts',
    20,
    'content',
    ['content:packages', 'assessments', 'community:posts'],
  ],
  [
    'video-session-service.ts',
    4,
    'content',
    ['content:lessons', 'watch-requests'],
  ],
  ['wallet-service.ts', 5, 'finance', ['finance:payroll', 'student:balance']],
  ['whatsapp-service.ts', 1, 'users', ['users']],
];

export const mutationContractRecords: readonly MutationContract[] =
  mutationContracts.map(([sourceFile, mutationCount, domain, keys]) => ({
    owner: sourceFile.replace(/-service\.ts$/, ''),
    sourceFile:
      `frontend/src/services/${sourceFile}` as MutationContract['sourceFile'],
    mutationCount,
...
export function validateQueryContracts(
  contracts: readonly QueryContract[] = queryContracts
): string[] {
  const errors: string[] = [];
  for (const [index, contract] of contracts.entries()) {
    if (
      ...
      errors.push(
        `contract[${index}] is missing owner, domain, scopes, keys, or operations`
      );
    }
  }
  const sourceFiles = new Set(
    mutationContractRecords.map((contract) => contract.sourceFile)
  );
  if (sourceFiles.size !== mutationContractRecords.length)
    errors.push('mutation contract source files must be unique');
  return errors;
}
