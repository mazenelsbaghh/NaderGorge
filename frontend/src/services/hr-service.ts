...
export interface AttendanceBreakDto { id: string; startedAt: string; endedAt?: string | null; }
export interface AttendanceSessionDto { id: string; workDate: string; clockedInAt: string; clockedOutAt?: string | null; state: string; workedMinutes: number; lateMinutes: number; earlyLeaveMinutes: number; overtimeMinutes: number; breaks?: AttendanceBreakDto[]; }
...
export const hrService = {
  ...
  startAttendanceBreak: async (): Promise<ApiResponse<string>> => {
    const res = await apiClient.post<ApiResponse<string>>('/hr/self/attendance/breaks/start', {}, { headers: { 'Idempotency-Key': createClientId() } }); return res.data;
  },
