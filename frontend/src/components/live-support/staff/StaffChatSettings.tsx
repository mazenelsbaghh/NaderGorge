...
export function StaffChatSettings({ open, preferences, onClose, onChange }: StaffChatSettingsProps) {
  ...
  return <section aria-labelledby="chat-settings-title" className="rounded-2xl border border-[var(--admin-border)] bg-[var(--admin-card)] p-4">
    ...
    <div className="mt-4 grid gap-4 lg:grid-cols-2">
      ...
      <fieldset><legend className="flex items-center gap-2 text-sm font-bold text-[var(--admin-text)]"><Volume2 className="size-4"/>نغمة التنبيه</legend><div className="mt-2 flex flex-wrap gap-2">{sounds.map((sound) => <button key={sound.value} type="button" onClick={() => { onChange({ sound: sound.value }); playLiveSupportSound(sound.value, preferences.soundVolume); }} className={`min-h-11 rounded-lg border px-3 text-sm font-semibold ${preferences.sound === sound.value ? 'border-[var(--admin-primary)] bg-[var(--admin-primary-15)] text-[var(--admin-primary)]' : 'border-[var(--admin-border)] text-[var(--admin-text)] hover:bg-[var(--admin-hover)]'}`}>{sound.label}</button>)}</div><label className="mt-4 block text-sm font-semibold text-[var(--admin-text)]" htmlFor="live-support-sound-volume"><span className="flex items-center justify-between gap-3"><span>مستوى الصوت</span><output htmlFor="live-support-sound-volume" className="tabular-nums text-[var(--admin-primary)]">{preferences.soundVolume}%</output></span><input id="live-support-sound-volume" aria-label="مستوى صوت تنبيه الدعم" type="range" min="0" max="100" step="1" value={preferences.soundVolume} onChange={(event) => onChange({ soundVolume: Number(event.target.value) })} className="mt-2 h-11 w-full accent-[var(--admin-primary)]"/></label><button type="button" onClick={() => playLiveSupportSound(preferences.sound, preferences.soundVolume)} className="mt-2 min-h-11 rounded-lg border border-[var(--admin-border)] px-3 text-sm font-semibold text-[var(--admin-text)] hover:bg-[var(--admin-hover)]">تجربة الصوت</button></fieldset>
    </div>
  </section>;
}
