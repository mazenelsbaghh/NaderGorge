import { SupportBlockPanel } from '@/components/live-support/admin/SupportBlockPanel';
...
export function StaffConversationWorkspace({ conversation, messages, draft, participantDraft, ownershipLost, pendingAction, messagesLoading, messagesError, hasOlderMessages, hasPendingMessageGap, olderMessagesLoading, olderMessagesError, onRetryMessages, onLoadOlderMessages, onDraftChange, onSend, onSendWhatsAppTemplate, uploading, onUpload, onEditMessage, onDeleteMessage, onTransfer, onClose, cannedReplies, onCannedReply, preferences, replyFocusRequest }: StaffConversationWorkspaceProps) {
  ...
  const activeReplyTarget = channelCapabilities.supportsMessageReply && replyTarget?.conversationId === conversation?.id ? replyTarget : undefined;
  ...
  const sendAndRestoreFocus = () => {
    if (!draft.trim() || !channelCapabilities.canSendFreeform) return;
  ...
  const confirmImageUpload = () => {
    if (!pendingImageForConversation || !channelCapabilities.canSendAttachments || ownershipLost || pendingAction || uploading) return;
  ...
  const isExternal = channelCapabilities.usesExternalThread;
  const channelPresentation = getLiveSupportChannelPresentation(conversation);
  ...
  const participantDetail = isWhatsApp
    ? 'محادثة واتساب'
    : isMessenger
    ? 'محادثة واردة من صفحة فيسبوك'
    : conversation.participantType === 'Guest'
    ? 'زائر، يحتاج ربطًا يدويًا فقط'
    : conversation.subject
      ? `طالب مسجل · ${conversation.subject}`
      : 'طالب مسجل';
  const externalWindowClosed =
    isExternal && channelCapabilities.customerServiceWindowOpen === false;
  const replyDisabled =
    ownershipLost ||
    Boolean(pendingAction) ||
    uploading ||
    !channelCapabilities.canSendFreeform;
    ...
    <header className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--admin-border)] p-4">
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          <h2 className="font-bold text-[var(--admin-text)]">{participantName}</h2>
        ...
        <p
          className="truncate text-xs text-[var(--admin-muted)]"
      ...
      {messagesLoading ? <p role="status" className="text-sm text-[var(--admin-muted)]">جارٍ تحميل الرسائل…</p> : messagesError ? <div role="alert" className="space-y-2 text-sm text-[var(--admin-danger)]"><p>{messagesError}</p><button type="button" onClick={onRetryMessages} className="font-semibold underline">إعادة المحاولة</button></div> : <>
        ...
        {messages.map((message, index) => {
          ...
          return <Fragment key={message.id}>
            ...
            <article dir="auto" data-live-support-message-id={message.id} aria-label={`رسالة من ${senderLabel}`} style={{ backgroundColor: colors.backgroundColor, color: colors.color, fontSize: fontSize(preferences.fontScale) }} className={`max-w-[76%] break-words [overflow-wrap:anywhere] rounded-2xl px-3 py-2 ${isStaffMessage ? 'mr-auto' : 'ml-auto'}`}><p className="mb-1 text-xs font-bold opacity-75">{senderLabel}</p>{message.replyTo ? <ReplyPreview message={message.replyTo}/>: null}<LiveSupportMessageContent message={message} audience="staff" staffWhatsAppThreadConversationId={isWhatsApp ? conversation.id : undefined}/>{channelCapabilities.supportsMessageReply && !message.deletedAt && message.conversationId === conversation.id ? <button type="button" onClick={() => { setReplyTarget(message); replyInputRef.current?.focus(); }} className="mt-1 inline-flex min-h-8 items-center gap-1 rounded-md px-1.5 text-xs font-bold opacity-75 hover:bg-black/10"><MessageSquareReply size={14}/>رد</button> : null}{isStaffMessage && channelCapabilities.supportsMessageMutation && message.conversationId === conversation.id ? <LiveSupportMessageActions message={message} onEdit={onEditMessage} onDelete={onDeleteMessage}/> : null}<LiveSupportMessageMeta message={message} audience="staff"/></article>
          </Fragment>;
        })}
        {channelCapabilities.supportsParticipantTypingPreview && participantDraft !== null && participantDraft !== undefined ? <article className="ml-auto max-w-[76%] rounded-2xl border border-[var(--admin-border)] bg-[var(--admin-primary-15)] px-3 py-2 text-sm text-[var(--admin-text)]"><p className="mb-1 text-xs font-bold text-[var(--admin-primary)]">{participantName} يكتب الآن…</p><p dir="auto" className="whitespace-pre-wrap break-words [overflow-wrap:anywhere]">{participantDraft || '…'}</p></article> : null}
      </>}
    </div>
    </div>
    <div className="shrink-0 border-t border-[var(--admin-border)] p-4">
      <SupportBlockPanel key={conversation.id} conversationId={conversation.id} />
      {isWhatsApp ? <WhatsAppTemplatePicker disabled={!channelCapabilities.canSendTemplate || ownershipLost || Boolean(pendingAction) || Boolean(uploading)} onSend={onSendWhatsAppTemplate}/> : null}
      {externalWindowClosed ? <p role="status" className="mb-3 rounded-lg bg-[var(--admin-warning-10)] px-3 py-2 text-sm font-medium text-[var(--admin-warning)]">{isWhatsApp ? 'انتهت نافذة الرد النصي. أرسل قالب واتساب معتمدًا لبدء المحادثة من جديد.' : 'انتهت نافذة الرد المتاحة لهذه الصفحة. انتظر رسالة جديدة من العميل قبل الرد.'}</p> : null}
      {!externalWindowClosed ? <>
      {isMessenger ? <p role="status" className="mb-2 text-xs font-medium text-[var(--admin-muted)]">الرد على ماسنجر متاح بالنص فقط حاليًا.</p> : null}
      ...
      {pendingImageForConversation && imagePreviewUrl && <div className="mb-3 flex flex-wrap items-center gap-3 rounded-xl border border-[var(--admin-border)] bg-[var(--admin-card-soft)] p-3" role="status" aria-label="معاينة المرفق قبل الإرسال">
        {pendingImageForConversation.type === 'application/pdf' ? <span className="p-4 font-bold">PDF</span> : <Image src={imagePreviewUrl} alt="معاينة الصورة قبل الإرسال" width={160} height={112} unoptimized className="h-28 w-40 rounded-lg bg-[var(--admin-card)] object-contain" />}
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-bold text-[var(--admin-text)]">{pendingImageForConversation.name}</p>
          <p className="mt-1 text-xs text-[var(--admin-muted)]">راجع المرفق قبل إرساله للعميل.</p>
          <div className="mt-3 flex flex-wrap gap-2">
            <button type="button" disabled={replyDisabled} onClick={confirmImageUpload} className="min-h-10 rounded-lg bg-[var(--admin-primary)] px-3 text-sm font-bold text-[var(--admin-primary-contrast)] disabled:opacity-50">{uploading ? 'جارٍ الإرسال…' : 'إرسال المرفق'}</button>
      ...
      {repliesOpen && <div id="staff-canned-replies" className="mb-2 max-h-48 overflow-y-auto rounded-xl bg-[var(--admin-card-soft)] p-2" aria-label="الردود الجاهزة">{cannedReplies.length === 0 ? <p className="px-3 py-4 text-center text-sm font-medium text-[var(--admin-muted)]">لا توجد ردود جاهزة لهذا الحساب بعد.</p> : cannedReplies.map(reply => <button key={reply.id} type="button" disabled={replyDisabled} onClick={() => { if (reply.sendImmediately) shouldRestoreReplyFocus.current = true; onCannedReply(reply); setRepliesOpen(false); }} className="block w-full rounded-lg px-3 py-2.5 text-right text-sm font-semibold text-[var(--admin-text)] hover:bg-[var(--admin-hover)] disabled:opacity-50"><span className="block truncate">{reply.title}</span><span className="mt-0.5 block truncate text-xs font-normal text-[var(--admin-muted)]">{reply.sendImmediately ? 'إرسال مباشر' : 'إضافة إلى مربع الكتابة'}</span></button>)}</div>}
      <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] gap-2 sm:grid-cols-[auto_auto_minmax(0,1fr)_auto]">
        <div className="col-span-3 sm:col-span-1">
          <button type="button" aria-expanded={repliesOpen} aria-controls="staff-canned-replies" disabled={replyDisabled} onClick={() => setRepliesOpen((open) => !open)} className="inline-flex h-11 w-full items-center justify-center gap-1.5 rounded-xl border border-[var(--admin-border)] bg-[var(--admin-card-soft)] px-3 text-xs font-bold text-[var(--admin-primary)] hover:bg-[var(--admin-hover)] disabled:opacity-50 sm:w-auto"><MessageSquareText size={17}/>ردود جاهزة<ChevronDown size={14} className={repliesOpen ? 'rotate-180 transition-transform' : 'transition-transform'}/></button>
        </div>
        <div className="flex shrink-0 gap-2">
          {channelCapabilities.supportsAttachments ? <label aria-label="اختيار صورة أو PDF" title="PDF حتى 90 ميجابايت، والصور حتى 10 ميجابايت" className={`grid size-11 shrink-0 place-items-center rounded-xl border border-[var(--admin-border)] text-[var(--admin-muted)] focus-within:outline-2 ${replyDisabled || pendingImageForConversation ? 'pointer-events-none opacity-50' : 'cursor-pointer hover:bg-[var(--admin-hover)]'}`}>{uploading ? <LoaderCircle className="animate-spin" size={18}/> : <Paperclip size={18}/>}<input type="file" accept="image/jpeg,image/png,image/webp,application/pdf" disabled={replyDisabled || Boolean(pendingImageForConversation)} onChange={(event) => { const file = event.target.files?.[0]; if (file && conversation) setPendingImage({ conversationId: conversation.id, file }); event.currentTarget.value = ''; }} className="sr-only"/></label> : null}
          <EmojiPicker tone="staff" disabled={replyDisabled} onSelect={insertEmoji}/>
        </div>
        <input ref={replyInputRef} aria-label="رد موظف الدعم" disabled={replyDisabled} value={draft} onChange={event => onDraftChange(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') { event.preventDefault(); sendAndRestoreFocus(); } }} className="h-11 min-w-0 rounded-xl border border-[var(--admin-border)] bg-[var(--admin-card)] px-3 text-[var(--admin-text)] outline-none placeholder:text-[var(--admin-muted)] focus-visible:border-[var(--admin-primary)] focus-visible:ring-2 focus-visible:ring-[var(--admin-primary-15)] disabled:bg-[var(--admin-card-soft)]" placeholder={ownershipLost ? 'المحادثة لم تعد مملوكة لك' : isWhatsApp ? 'اكتب ردك على واتساب' : isMessenger ? 'اكتب ردك على ماسنجر' : 'اكتب ردك للطالب'}/>
      ...
      {channelCapabilities.supportsAttachments ? <StaffVoiceRecorder disabled={ownershipLost || Boolean(pendingAction) || !channelCapabilities.canSendAttachments} uploading={uploading} onSend={onUpload} /> : null}
      </> : null}
    </div>
  </main>;
}
