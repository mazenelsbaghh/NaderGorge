...
import { renderWhatsAppTemplatePreview, whatsAppTemplateParameterCount } from '@/components/live-support/staff/whatsapp-template';
...
export function WhatsAppTemplatePicker({ disabled, onSend }: WhatsAppTemplatePickerProps) {
  ...
  const parameterCount = useMemo(() => selected ? whatsAppTemplateParameterCount(selected) : 0, [selected]);
  ...
  useEffect(() => {
    setParameters(Array.from({ length: parameterCount }, () => ''));
  }, [parameterCount, selectedId]);
  ...
  async function loadTemplates() {
    ...
    try {
      ...
      const approved = available.filter(template => template.status.toUpperCase() === 'APPROVED');
      setTemplates(approved);
      setSelectedId(current => approved.some(template => template.id === current) ? current : approved[0]?.id ?? '');
    } catch { setError('تعذر تحميل قوالب واتساب.'); }
    finally { setLoading(false); }
  }

  async function send() {
    if (!selected || parameters.some(value => !value.trim())) return;
    setSending(true);
    setError('');
    try {
  ...
  return <section className="mb-3 rounded-xl bg-[var(--admin-card-soft)] p-3" aria-label="إرسال قالب واتساب">
    ...
    <div className="mt-3 flex flex-wrap gap-2">
      <select aria-label="قالب واتساب" disabled={loading || sending} value={selectedId} onChange={event => setSelectedId(event.target.value)} className="min-h-11 min-w-56 flex-1 rounded-xl border border-[var(--admin-border)] bg-[var(--admin-card)] px-3 text-sm text-[var(--admin-text)] focus-visible:outline-2 focus-visible:outline-[var(--admin-primary)]">
    ...
    {selected ? <div className="mt-3 space-y-2">
      {parameters.map((value, index) => <label key={index} className="block text-xs font-bold text-[var(--admin-text)]">المتغير {index + 1}<input value={value} onChange={event => setParameters(current => current.map((entry, entryIndex) => entryIndex === index ? event.target.value : entry))} maxLength={1000} className="mt-1 h-11 w-full rounded-xl border border-[var(--admin-border)] bg-[var(--admin-card)] px-3 text-sm font-normal outline-none focus-visible:border-[var(--admin-primary)] focus-visible:ring-2 focus-visible:ring-[var(--admin-primary-15)]"/></label>)}
      ...
      <button type="button" disabled={sending || parameters.some(value => !value.trim())} onClick={() => void send()} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[var(--admin-primary)] px-4 font-bold text-[var(--admin-primary-contrast)] disabled:opacity-50">{sending ? <LoaderCircle className="animate-spin" size={17}/> : <MessageCircle size={17}/>}إرسال القالب</button>
    </div> : null}
