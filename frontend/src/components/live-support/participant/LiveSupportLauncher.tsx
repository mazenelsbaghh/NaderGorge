...
export function LiveSupportLauncher() {
  ...
  async function refresh() {
    ...
    try {
      ...
      setNeedsGuest(false);
      if (current) setMessages(await liveSupportService.getMessages(current.id));
  ...
  async function send() {
    ...
    try { const message = await liveSupportService.sendParticipantMessage(conversation.id, { clientMessageId: crypto.randomUUID(), type: 'Text', content: value }); setMessages((items) => [...items, message]); }
