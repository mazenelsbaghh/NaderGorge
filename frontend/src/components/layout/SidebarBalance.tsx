  ...
  return (
    ...
    >
      {loading ? (
        ...
        <>
          <Wallet className="absolute top-1.5 h-3.5 w-3.5 opacity-50 group-hover:opacity-100 transition-opacity group-data-[expanded=true]/sidebar:static group-data-[expanded=true]/sidebar:h-5 group-data-[expanded=true]/sidebar:w-5 flex-shrink-0" />
          <span className="absolute bottom-1.5 flex max-w-[2.85rem] items-baseline justify-center gap-0.5 overflow-hidden font-sans text-sm font-black leading-none tracking-tight group-data-[expanded=true]/sidebar:static group-data-[expanded=true]/sidebar:max-w-none group-data-[expanded=true]/sidebar:text-sm flex-shrink-0">
            <span className="truncate group-data-[expanded=true]/sidebar:hidden">{formatCompactBalance(totalAvailableBalance)}</span>
            <span className="hidden truncate group-data-[expanded=true]/sidebar:inline">{formatFullBalance(totalAvailableBalance)}</span>
            <span className="text-[8px] font-bold group-data-[expanded=true]/sidebar:text-sm">ج.م</span>
          </span>
          <span className="hidden group-data-[expanded=true]/sidebar:block text-xs font-bold text-[var(--admin-muted)] truncate whitespace-nowrap me-auto">
            إجمالي المتاح
          </span>
        </>
      )}
