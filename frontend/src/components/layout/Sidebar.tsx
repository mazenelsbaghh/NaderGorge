...
export function Sidebar({ items, title }: SidebarProps) {
  ...
  return (
    <aside className="sticky top-0 hidden h-screen w-72 shrink-0 flex-col border-e border-[var(--admin-border)] bg-[var(--admin-sidebar)] px-4 py-5 backdrop-blur md:flex">
      {title && (
        <div className="mb-4 rounded-2xl border border-[var(--admin-border)] bg-[var(--admin-card)] p-4 shadow-sm">
      ...
      <nav className="flex-1 space-y-2 rounded-2xl border border-[var(--admin-border)] bg-[var(--admin-card)] p-3 shadow-sm">
        {items.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            ...
            className={cn(
              "flex items-center gap-3 rounded-2xl px-4 py-3.5 text-sm font-bold transition-[color,background-color,border-color,opacity,transform,box-shadow]",
              pathname === item.href
