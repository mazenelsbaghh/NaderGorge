...
const GROUP_CONFIG = [
  ...
  },
  {
    ...
    icon: Briefcase,
    hrefs: ['/admin/operations', '/admin/hr', '/admin/finance', '/admin/shared-packages', '/admin/wallets', '/admin/recharge-verification'],
  },
  {
    id: 'crm_chat',
...
export function AdminShellChrome({
  ...
  const navGroups = useMemo(() => GROUP_CONFIG.map((group) => {
    const items = filteredNavItems.filter((item) =>
      ...
      (!normalizedNavQuery || item.label.toLocaleLowerCase('ar').includes(normalizedNavQuery))
    );
    return {
      ...group,
      items,
  ...
  const mobileNavGroups = useMemo(() => GROUP_CONFIG.map((group) => ({
    ...
    items: mobileMoreItems.filter((item) =>
      group.hrefs.includes(item.href) &&
      (!normalizedMobileNavQuery || item.label.toLocaleLowerCase('ar').includes(normalizedMobileNavQuery))
    ),
