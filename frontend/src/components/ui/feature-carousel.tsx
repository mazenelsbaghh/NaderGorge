...
const StepImage = forwardRef<
  HTMLImageElement,
  StepImageProps & ComponentPropsWithoutRef<typeof Image>
>(
  (
      ...
      >
        <div className="m-10 min-h-[450px] w-[calc(100%-5rem)]">
          <AnimatePresence mode="wait">
            ...
            >
              <motion.h2
                className="text-xl font-bold tracking-tight text-[var(--admin-text)] md:text-2xl"
                initial={{ opacity: 0, x: -20 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{
                  delay: 0.1,
...
export function FeatureCarousel({
  ...
  useEffect(() => {
    if (typeof controlledStep === "number") {
      setCurrentNumber(controlledStep)
    }
  ...
  const handleIncrement = () => {
    ...
    setIsAnimating(true)
    if (typeof controlledStep === "number" && onStepChange) {
      onStepChange((controlledStep + 1) % totalSteps)
      return
    }
    increment()
  }
  ...
  const handleStepChange = (index: number) => {
    if (isAnimating || index === step) return
    setIsAnimating(true)
    if (typeof controlledStep === "number" && onStepChange) {
      onStepChange(index)
      return
          ...
          return (
            ...
            >
              <AnimatedStepImage
                ...
                className={clsx(
                  step4imgClass,
                  "pointer-events-none top-[50%] overflow-hidden rounded-2xl border border-neutral-100/10 dark:border-zinc-700"
                )}
