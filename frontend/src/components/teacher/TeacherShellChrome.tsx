...
export function TeacherShellChrome({
  ...
  return (
    ...
    >
      ...
      <main className="app-shell-scroll relative z-10 h-dvh overflow-y-auto overscroll-y-auto px-4 py-6 pb-[calc(8rem+env(safe-area-inset-bottom))] lg:mr-24 lg:px-7 lg:py-8 lg:pb-10">
      ...
      {isMobileMenuOpen ? (
        <>
          <button
            ...
            aria-label="إغلاق قائمة المدرس"
            onClick={() => setIsMobileMenuOpen(false)}
          />
          <aside
            className="fixed bottom-0 right-0 z-[60] max-h-[80vh] w-full overflow-y-auto rounded-t-[24px] border border-[var(--admin-border)] bg-[var(--admin-sidebar)] px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-4 shadow-[0_-24px_70px_var(--admin-shadow)] lg:hidden"
            aria-label="قائمة المدرس الإضافية"
          >
