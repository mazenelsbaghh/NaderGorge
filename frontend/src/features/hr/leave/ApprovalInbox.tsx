...
import { cairoDateTimeLocalToUtcISOString, formatCairoDateTime } from '@/lib/cairo-time';
...
function formatDate(value?: string | null) {
  return value ? formatCairoDateTime(value) : 'غير متاح';
}
  ...
  async function delegate(event: FormEvent) {
    ...
    try {
      await hrService.createApprovalDelegation({
        ...delegation,
        scope: 'leave',
        startsAt: cairoDateTimeLocalToUtcISOString(delegation.startsAt),
        endsAt: cairoDateTimeLocalToUtcISOString(delegation.endsAt),
      });
            ...
            <article key={row.id} className="admin-panel">
              <div className="flex flex-wrap justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-[var(--admin-muted)]">
                    {row.step} · الاستحقاق {formatCairoDateTime(row.dueAt, { dateStyle: 'medium', timeStyle: 'short' })}
                  </p>
                </div>
