              ...
              <table className="w-full min-w-[840px] text-right text-sm">
                ...
                <tbody className="divide-y divide-[var(--admin-border)]">
                  {agreements.map((agreement) => <tr key={agreement.id} className={!agreement.isActive ? 'opacity-55' : 'hover:bg-[var(--admin-hover)]'}>
                    ...
                    <td className="px-4 py-3 text-xs font-bold text-[var(--admin-muted)]">من {new Date(agreement.effectiveFrom).toLocaleDateString('ar-EG', { timeZone: 'Africa/Cairo' })}<br />{agreement.effectiveTo ? `حتى ${new Date(agreement.effectiveTo).toLocaleDateString('ar-EG', { timeZone: 'Africa/Cairo' })}` : 'مستمر'}</td>
