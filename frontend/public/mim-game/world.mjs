...
function relic(parent, index, position, title) {
  ...
  label(group, title, [0, 0.85, 0.977], 1.75);
  return ring;
}
...
export function createWorld(container, callbacks, content) {
  ...
  const doors = museum(scene, content);
  ...
  const rings = spots.map((position, index) =>
    relic(scene, index, position, content.missions[index].title)
  );
  ...
  container.addEventListener(
    ...
    (event) => {
      ...
      if (puzzleActive) {
        if (Math.abs(deltaX) > 3) callbacks.rotate(deltaX > 0 ? 1 : -1);
      } else {
        angle -= deltaX * 0.008;
  ...
  container.addEventListener(
    ...
    (event) => {
      ...
      if (puzzleActive) {
        if (mechanism.hit(raycaster)) callbacks.rotate(1);
        return;
      }
    ...
    render(position, moving, completed, delta) {
      ...
      doors.forEach((door, index) => {
        door.rotation.y = T.MathUtils.lerp(
          door.rotation.y,
          completed === 3 ? (index === 0 ? 1.35 : -1.35) : 0,
          1 - Math.exp(-delta * 4)
        );
      });
