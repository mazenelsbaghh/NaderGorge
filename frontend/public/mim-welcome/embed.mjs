...
const skipTimer = setTimeout(() => { skip.hidden = false; }, 60000);
function stop() {
  dismissed = true;
  voice.pause();
...
function dismiss(completed = false) {
  if (dismissed) return;
  stop();
  parent.postMessage({ type: completed ? "mim-complete" : "mim-dismiss" }, location.origin);
}
function tick() {
  if (dismissed) return;
  ...
  if (elapsed >= duration) return dismiss(true);
