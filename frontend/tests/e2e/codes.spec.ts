...
test.describe('Access Codes Generation & Redemption Flow', () => {
  ...
  test.beforeEach(async ({ request }) => {
    ...
    await request.post('http://localhost:5245/api/e2e/clear-devices', { data: { phoneNumber: '20000000000' } });
    await request.post('http://localhost:5245/api/e2e/clear-devices', { data: { phoneNumber: '20000000001' } });
  ...
  test('T014: Admin can access codes page', async ({ browser }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Access Codes' })).toBeVisible({ timeout: 10000 });
