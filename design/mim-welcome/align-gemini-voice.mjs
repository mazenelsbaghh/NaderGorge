...
async function alignVoice(name) {
 ...
 const response = await client.models.generateContent({
  ...
  contents:[{inlineData:{mimeType:'audio/mpeg',data:audio.toString('base64')}},{text:'Listen carefully to this generated Egyptian Arabic welcome. Return JSON with a segments array: {start: seconds as a number, end: seconds as a number, text: verbatim Arabic}. Split exactly at these 7 phrases: "إيه ده! إنت وصلت؟", "نوّرت مسار!", "أنا ميم، وصاحبك في رحلة التعلّم.", "هنفهم، ونجرّب، ونحلّ سوا.", "جاهز؟", "يلا نبدأ أول خطوة!", "أشوفك جوّه!". Use actual timestamps from the attached recording, not word count estimates. Also add a short quality field describing audible language/dialect, clarity and any omitted or added text. No extra markdown.'}],
  config:{responseMimeType:'application/json'},
 });
 const alignment=JSON.parse(response.text);
 if(!Array.isArray(alignment.segments)||alignment.segments.length!==7)throw new Error('Unexpected alignment segments');
...
try{await Promise.all(['puck','charon'].map(alignVoice));}
