...
## Financial reporting reconciliation (2026-09-24, local changes)

- Unified the platform profits page, financial dashboard, income reports and ledger exports around recorded journal entries and Cairo calendar boundaries. Balance-sheet cards include opening balances; income and expenses use the selected period. Original and reversal entries both remain in history, on their respective dates.
- Kept the 19 September agreement recalculation as an explicit historical comparison rather than overwriting posted profit totals. Fixed inherited package agreements, explicit refund allocations, refund reversals, partial-refund cancellation and shared-sale rounding in that comparison.
- Teacher summaries separate sales, refunds, payouts and other adjustments. New teacher payouts and settlement payments record balanced journal entries within their existing transaction; the historical reconstruction workflow now includes net settlement payments and uses the same idempotency keys.
- Prevented inconsistent midpoint rounding from accepting an unbalanced journal. Refund posting now has one implementation, including refunds borne entirely by the teacher or platform.
- Verification: 106 focused Application tests passed, with 9 finance-repair cases skipped because `FINANCE_REPAIR_TEST_DB` is unset. Next.js production build, TypeScript checking and focused ESLint passed. Two platform-profits browser tests passed using installed Chrome and synthetic HTTP responses; they are UI/CSV evidence, not authenticated production evidence.
- PostgreSQL concurrency verification remains blocked: `TeacherAccountingConcurrencyTests` cannot construct its fixture without `ConnectionStrings__DefaultConnection`. The broad Finance integration attempt passed 21 tests and failed fixture setup for that one test. No real PostgreSQL concurrency, live financial totals, historical balance correction or production deployment is claimed. Existing unrelated workspace changes were preserved.

## Simple everyday finance screens (2026-09-24, local changes)

- Replaced the wide teacher-profit table with expandable teacher balances and a shared, plain-language income/expenses/profit summary. Cairo date presets make today and the current month one-click selections. Negative profit is explicitly labelled as a loss; negative teacher balances show an amount owed by the teacher.
- Kept exports, reconciliation differences and historical comparisons accessible in details. The balances screen separates recorded cash from profit, loads journal details only on request, and links to the existing wallet report. Advanced accounting navigation is grouped separately; route permissions are unchanged.
- Simplified expense entry to amount, category, description and payment status. A paid expense requires a treasury source; an unpaid expense is posted as a payable. A failed posting can retry the same saved expense and key within the open form. Corrected expense/refund creation handling to use the API's actual flat `id` response.
- This changes local application code only. It does not migrate historical financial data or publish a production release. Browser verification uses synthetic HTTP responses, not live balances or payment actions.
- Verification: Next.js production build and TypeScript passed; focused ESLint and route-permission contracts passed. Six installed-Chrome browser scenarios passed, covering date/teacher filtering and CSV, mobile failure/retry, losses and negative teacher balances, isolated journal failure, and paid/unpaid expense posting retries. Desktop and mobile screenshots were inspected. The temporary local browser configuration was removed after verification.


## One teacher account and withdrawal policy (2026-09-24, local changes)

- Added one teacher account snapshot and withdrawal policy used by the admin account, teacher self-service, platform profit report and legacy financial-summary entry points. Teacher-domain records remain authoritative for entitlement and withdrawals; journal balances and historical recalculations remain explicit reconciliation views. Profit-report reconciliation compares the journal with the teacher balance after open debt.
- The snapshot separates recognized earnings, actual cash paid, gross account balance, active reservations, open debt, debt already reserved in settlements, and the amount available for a new withdrawal. Signed source allocations retain their original agreement terms; pending/rejected allocations do not inflate earnings or calendar totals. Source/balance discrepancies are visible and do not silently rewrite historical money.
- Consolidated agreements, income sources, calculation explanations, withdrawal approvals/payments, paged settlement history and reversals in the teacher account screen. Legacy admin entry points lead there. Shared account summaries and the teacher withdrawal form use the same returned snapshot. Fixed amounts are no longer labelled as percentages. Existing-teacher agreement editing points to this screen; old transactions retain their recorded terms.
- Withdrawal requests subtract unreserved debt exactly once and reject fractional amounts below a cent. Settlement creation cannot bypass partially reserved debt; payment rechecks changed debt before cash is paid. Serialization retries discard rolled-back tracked entities so a retry cannot create a second payout. Cairo date filters include the entire selected final day.
- Verification: 113 focused Finance Application tests passed, including signed sources, pending review, partial debt reservation, changed-debt payment rejection, profit-report reconciliation and a failed-save retry. API and Next.js production builds, focused ESLint and route-permission checks passed. Nine distinct installed-Chrome scenarios passed with synthetic HTTP responses, including the legacy account redirect, approval versus payment, account load failure, teacher self-service withdrawal limit and responsive layouts. Desktop/mobile account screenshots were inspected.
- Refreshed endpoint and Admin AI inventories to match the current shared working tree, preserving its unrelated concurrent changes and blocked AI activation state. The runtime inventory tests passed with temporary process-only test secrets. Financial database records were not changed; no production deployment, live-balance reconciliation or PostgreSQL concurrency evidence is claimed.

## Unified teacher agreements and prepaid content-code batches (2026-09-24, local changes)

- Added one effective-dated agreement for purchases, code activation, and delivery. New unified agreements retire the older rules for the same scope while preserving captured transactions; the migration consolidates only complete, identical legacy rule sets. Different terms remain explicit. Existing Nader activation-only policy is preserved.
- Removed the obsolete generation-time commission calculation and the separate profit controls in code pages. Batch previews and activations now use the shared content-pricing and teacher-agreement sources. Unconfirmed delivery batches cannot be redeemed; billed batches cannot be deleted or billed again.
- The teacher account now owns batch timing, delivery, full/partial/deferred collection, immutable receipt references, and remaining amounts. Teacher-retained earnings are recognized once but excluded from withdrawal funds. Treasury receipts reduce the platform receivable without another sale or entitlement. Receivables are shown separately and are not automatically offset against other withdrawals.
- Added explicit teacher-retained labels and columns to account, period, and daily exports. Daily export summaries follow the same recognized-status policy as the account. Historical delivery records without collection evidence remain unclassified rather than being rewritten as paid or due.
- Added EF migration `20260924152917_UnifiedTeacherCodeCollections`; exercised the full migration chain on an isolated disposable PostgreSQL 16 database and verified consolidation preserves old agreement rows. Concurrent receipts retry after serialization conflicts and cannot collect more than the batch balance.
- Verification: 129 focused finance/code tests passed, including the real PostgreSQL migration/concurrency case; the five batch/account scenarios also passed after adding Excel export assertions. Ten Chrome browser scenarios passed against the production frontend build (mock HTTP data); mobile batch layout inspected. API/Next production builds, focused ESLint, 3 runtime inventory tests, and 16 inventory/contract tests passed. AdminAI activation remains blocked by its existing coverage gate.
- No release or production migration was performed. The newly implemented batch collection flow covers content access codes. The owner's answer confirming full/partial/deferred payment was incorporated; the clarification about content codes versus wallet-recharge codes is still unanswered, so wallet recharge accounting was not reclassified.
