...
def test_fee_activation_preserves_history_and_gate_detects_wrong_terms(corrupt_amount: bool) -> None:
    fixture = (Path(__file__).parent / "fixtures/finance-activation.sql").read_text()
    ...
    verification = (ROOT / "artifacts/teacher-defaults/verify-migration.sql").read_text().split("SELECT t.", 1)[0]
