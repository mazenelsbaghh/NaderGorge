...
def test_root_helper_switches_an_existing_pointer_atomically_and_idempotently(
    ...
    def release(release_id: str, padding: int = 0) -> tuple[Path, str]:
    ...
    new_root, new_digest = release("src-" + "a" * 40, 1024 * 1024)
    assert (new_root / "manifest.json").stat().st_size > 1024 * 1024
...
def test_root_helper_rejects_manifest_above_four_megabyte_bound(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    base = tmp_path / "opt/massar/releases"
    release_root = base / ("src-" + "b" * 40)
    release_root.mkdir(parents=True)
    manifest = release_root / "manifest.json"
    manifest.write_bytes(b"x" * (root_helper.MAXIMUM_MANIFEST_BYTES + 1))
    cluster_marker = tmp_path / "cluster-id"
    cluster_marker.write_text("massar-production\n", encoding="ascii")
    monkeypatch.setattr(root_helper, "BASE", base)
    monkeypatch.setattr(root_helper, "CLUSTER_MARKER", cluster_marker)
    ...
    with pytest.raises(
        root_helper.NormalizationError,
        match="bounded regular file",
    ):
        root_helper.inspect_release(release_root.name, "c" * 64)
