...
def install(inventory,transport) -> None:
    ...
    module = ROOT / "deploy/production/scripts/optimized_builder.py"
    staged_module = "/tmp/massar-optimized-builder.py"
    transport.copy(remote, module, staged_module, timeout_seconds=120)
    module_script = (
        "set -euo pipefail; "
        "trap 'rm -f /tmp/massar-optimized-builder.py' EXIT; "
        f"printf '%s  %s\\n' '{digest(module)}' '{staged_module}' | sha256sum -c -; "
        "sudo /usr/bin/install -d -m 0755 -o root -g root /usr/local/lib/massar; "
        f"sudo /usr/bin/install -m 0644 -o root -g root {staged_module} /usr/local/lib/massar/optimized_builder.py"
    )
    transport.run(remote, ("bash", "-lc", module_script), timeout_seconds=60)
def arguments():
