...
def run_remote_builder_workflow(*, repository: Path, output: Path, inventory: object, transport: object, provenance: Mapping[str,Any], created_at: str) -> dict[str,Any]:
    ...
    try:
        ...
        write_json_atomic(manifest,final); assert_source_unchanged(repository, dict(provenance)); publish_final_manifest(temporary,remote.release_id,nodes,inventory.cluster["ssh_user"],transport)
