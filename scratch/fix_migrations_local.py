...
def check_table_exists(table_name):
    ...
    cmd = f'docker exec -i massar_db psql -U postgres -d masar_platform -t -A -c "{query}"'
...
def main():
    ...
    print("🛠 Running docker/create_missing_tables.sql to ensure missing tables exist on local database...")
    sql_path = "./docker/create_missing_tables.sql"
    if os.path.exists(sql_path):
        try:
            ...
            if code != 0:
                ...
                print("✅ create_missing_tables.sql applied successfully on local database!")
                if out.strip():
                    print(out.strip())
    ...
    # 1. Dynamically scan migrations in the repository
    ...
    if len(verified_applied) > 0:
        ...
        print("🛠  Populating __EFMigrationsHistory on local database (using stdin)...")
        db_cmd = 'docker exec -i massar_db psql -U postgres -d masar_platform'
    ...
    print("⚙️  Running pending migrations on local server...")
    migrator_cmd = "docker compose --profile migration run --rm migrator"
    ...
    if code == 0:
        print("🎉 Local migrations fixed and applied successfully!")
    else:
        print("❌ Migrator returned errors.")
        sys.exit(code)
