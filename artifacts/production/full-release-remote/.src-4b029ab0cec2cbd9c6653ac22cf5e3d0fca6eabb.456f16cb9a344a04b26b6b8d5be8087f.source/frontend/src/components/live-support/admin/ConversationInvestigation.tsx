...
export function ConversationInvestigation({ timeline, close }: { timeline: LiveSupportConversationTimeline; close: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] grid place-items-center bg-[color-mix(in_srgb,var(--admin-primary)_72%,transparent)] p-4" onClick={close}>
      <section role="dialog" aria-modal="true" aria-label="متابعة المحادثة" onClick={(event) => event.stopPropagation()} className="flex max-h-[92dvh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-[var(--admin-border)] bg-[var(--admin-card)] shadow-[var(--admin-shadow)]" dir="rtl">
