...
export default function ParentPortalPageClient() {
  ...
  const handleVerify = async (event: FormEvent) => {
    ...
    setError('');
    try {
      const result = await parentService.verifyCode(normalized);
      sessionStorage.setItem(TOKEN_KEY, result.token);
      setStudentName(result.studentName);
      await loadDetails(result.token);
    } catch (cause) {
  ...
  if (loading) {
    return <main className="grid min-h-screen place-items-center bg-[var(--landing-bg)] text-[var(--landing-ink)]"><span className="text-sm font-bold">جاري فتح المتابعة...</span></main>;
  }
    ...
    return (
      <main className="min-h-screen bg-[var(--landing-bg)] px-4 pb-6 pt-28 text-[var(--landing-ink)] sm:px-6 sm:pt-32">
        <div className="mx-auto flex w-full max-w-lg flex-col gap-10 pt-4 sm:pt-12">
          ...
          <section className="rounded-2xl bg-white p-6 shadow-[0_8px_24px_rgba(10,29,61,0.1)] sm:p-8" dir="rtl">
            ...
            <form onSubmit={handleVerify} className="mt-7 space-y-4">
              ...
              <input id="parent-tracking-code" autoComplete="one-time-code" inputMode="text" maxLength={6} value={trackingCode} onChange={(event) => setTrackingCode(event.target.value.replace(/[^a-zA-Z0-9]/g, '').toUpperCase())} placeholder="مثال: A1B2C3" className="w-full rounded-xl border border-[var(--landing-line)] bg-[var(--landing-bg)] px-4 py-3 text-center text-xl font-black tracking-[0.24em] uppercase outline-none transition focus:border-[var(--landing-accent)] focus:ring-2 focus:ring-[var(--landing-accent)]/20" />
            ...
            <p className="mt-5 text-xs font-semibold leading-6 text-[var(--landing-muted)]">ستجد الرمز في حساب الطالب، ويمكنه مشاركته مع ولي الأمر.</p>
          </section>
        </div>
      </main>
    );
    ...
    <main className="min-h-screen bg-[var(--landing-bg)] px-4 pb-6 pt-28 text-[var(--landing-ink)] sm:px-6 sm:pt-32" dir="rtl">
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="flex flex-wrap items-center justify-between gap-4 border-b border-[var(--landing-line)] pb-5">
          ...
          <button type="button" onClick={() => { sessionStorage.removeItem(TOKEN_KEY); setDetails(null); setTrackingCode(''); }} className="inline-flex items-center gap-2 rounded-xl border border-[var(--landing-line)] bg-white px-4 py-2.5 text-sm font-bold transition hover:bg-[var(--landing-bg-soft)]"><LogOut className="h-4 w-4" />تغيير الطالب</button>
        </header>
