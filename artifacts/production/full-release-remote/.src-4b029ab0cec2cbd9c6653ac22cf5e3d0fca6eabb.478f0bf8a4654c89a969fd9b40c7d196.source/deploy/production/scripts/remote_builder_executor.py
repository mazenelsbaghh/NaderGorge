...
REDIS_PASSWORD_FILE = Path("/etc/massar/secrets/redis-password")
...
def redis_role() -> str:
    """Query local Redis ROLE using the established root-only secret pattern."""
    try:
        password = REDIS_PASSWORD_FILE.read_text(encoding="utf-8").strip()
        if not password:
            raise OSError("empty Redis password file")
        output = subprocess.check_output(
            ["/usr/bin/redis-cli", "-h", "127.0.0.1", "-p", "6379", "--raw", "ROLE"],
            stderr=subprocess.DEVNULL,
            timeout=5,
            env={**os.environ, "REDISCLI_AUTH": password},
        ).decode("utf-8", errors="strict").splitlines()
    except (OSError, subprocess.SubprocessError, UnicodeDecodeError) as exc:
        raise RemoteBuilderError("cannot safely determine local Redis role") from exc
    if not output or output[0] not in {"master", "slave", "replica"}:
        raise RemoteBuilderError("cannot safely determine local Redis role")
    return output[0]


def preflight(*, workspace: Path, release_id: str, expected_source_sha256: str, patroni_role_reader=None, redis_role_reader=None) -> Path:
    ...
    redis_role_reader = redis_role if redis_role_reader is None else redis_role_reader
    ...
    if redis_role_reader() == "master":
        raise RemoteBuilderError("remote builder refuses the current Redis master")
