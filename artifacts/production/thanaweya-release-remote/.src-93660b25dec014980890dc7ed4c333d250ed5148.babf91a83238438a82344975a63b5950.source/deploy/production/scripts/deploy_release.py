...
def recover_failed_node(
    *,
    ...
    previous_release = recover_node(
        transport, target, node, release_id
    )
    traffic_writer(
    ...
    try:
        for node_id in ROLLING_ORDER:
            ...
            try:
                ...
                try:
                    previous_release = recover_failed_node(
                        transport=transport,
                        target=target,
                        node=node,
                    ...
                    recovery_marker = safe_failure_marker(
                        recovery_exc, "RECOVERY"
                    )
                    raise DeployError(
                ...
                deploy_marker = safe_failure_marker(exc, "DEPLOY")
                raise DeployError(
                    f"rollout failed on {node_id}; restored and undrained "
                    f"previous release {previous_release}; {deploy_marker}"
                ) from exc
            traffic(
                root, args.inventory, args.known_hosts,
