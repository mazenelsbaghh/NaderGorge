...
export async function seedE2E(request: APIRequestContext, reason = 'E2E backend/seed is unavailable', clearDatabase = false) {
  ...
  try {
    response = await request.post(`${apiUrl}/e2e/seed`, {
      headers: { 'X-E2E-Token': process.env.E2E_TEST_TOKEN || 'E2eOnlyTestTokenValue123456789012345' },
      data: { clearDatabase, seedAdmin: true, seedStudents: true, seedAssistant: true, seedLiveSupport: true },
    });
  } catch {
