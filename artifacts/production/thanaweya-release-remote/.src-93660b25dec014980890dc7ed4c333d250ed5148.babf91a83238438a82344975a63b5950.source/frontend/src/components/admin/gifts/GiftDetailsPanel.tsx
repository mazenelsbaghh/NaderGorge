...
export function GiftDetailsPanel({ gift, onChanged }: { gift: GiftDetailsDto; onChanged: () => Promise<void> }) {
  ...
  return <div className="space-y-6">
    ...
    {showRevoke ? <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/45 p-4"><div role="dialog" aria-modal="true" aria-labelledby="revoke-title" className="w-full max-w-lg rounded-lg bg-[var(--admin-card)] p-5 shadow-2xl"><h2 id="revoke-title" className="text-lg font-black text-[var(--admin-text)]">إلغاء المتبقي من الهدية</h2><p className="mt-2 text-sm text-[var(--admin-muted)]">الاستخدام السابق سيظل محفوظاً، وسيُمنع فقط الوصول أو الرصيد غير المستخدم.</p><textarea autoFocus value={reason} onChange={(event) => setReason(event.target.value)} className="admin-input mt-4 min-h-24" maxLength={500} placeholder="سبب الإلغاء" /><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => setShowRevoke(false)} className="h-10 rounded-lg px-4 text-sm font-bold text-[var(--admin-muted)]">تراجع</button><button type="button" onClick={() => void revoke()} disabled={saving || !reason.trim()} className="inline-flex h-10 items-center gap-2 rounded-lg bg-red-600 px-4 text-sm font-bold text-white disabled:opacity-50">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Ban className="h-4 w-4" />} تأكيد الإلغاء</button></div></div></div> : null}
  </div>;
}
