...
public sealed record PayrollCalculationInput(Guid EmployeeId, decimal BaseSalary, int LateMinutes, int AbsenceDays,
    int OvertimeMinutes, IReadOnlyDictionary<string, decimal>? Variables = null, int EarlyLeaveMinutes = 0);
...
public sealed class PayrollCalculationEngine
{
    ...
    private static readonly HashSet<string> AttendanceExpressions = ["attendance.late_minutes * rate", "attendance.absence_days * rate", "attendance.overtime_minutes * rate", "attendance.early_leave_minutes * rate"];
    ...
    public PayrollCalculationResult Calculate(PayrollCalculationInput input, IEnumerable<PayrollRule> rules)
    {
        ...
        foreach (var rule in rules.Where(item => item.IsActive).OrderBy(item => item.Priority).ThenBy(item => item.Id))
        {
            ...
            var inputs = JsonSerializer.Serialize(new { input.BaseSalary, input.LateMinutes, input.AbsenceDays, input.OvertimeMinutes, input.EarlyLeaveMinutes, rule.Rate, variables = input.Variables });
    ...
    private static decimal Evaluate(PayrollRule rule, PayrollCalculationInput input)
    {
        ...
        if (rule.Expression == "attendance.early_leave_minutes * rate") return input.EarlyLeaveMinutes * rule.Rate;
