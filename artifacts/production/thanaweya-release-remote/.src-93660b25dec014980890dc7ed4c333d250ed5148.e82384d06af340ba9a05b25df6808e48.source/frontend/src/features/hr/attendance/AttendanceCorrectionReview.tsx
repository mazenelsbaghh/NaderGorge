...
export function AttendanceCorrectionReview() {
  ...
  return <div className="space-y-3">{rows.length === 0 ? <div className="admin-panel py-14 text-center font-bold text-[var(--admin-muted)]">لا توجد تصحيحات للمراجعة.</div> : rows.map((row) => { const before = parseBeforeEvidence(row.beforeJson); return <article key={row.id} className="admin-panel"><div className="flex flex-wrap justify-between gap-3"><div><p className="font-black">{row.employee}</p><p className="text-sm text-[var(--admin-muted)]">{row.reason}</p></div><span className="admin-badge">{row.state}</span></div><div className="mt-4 grid gap-3 rounded-2xl bg-[var(--admin-card-soft)] p-4 sm:grid-cols-2"><div><p className="text-xs font-black text-[var(--admin-muted)]">قبل</p><p className="mt-1 text-sm font-bold">انصراف: {String(before.clockedOutAt ?? 'غير مسجل')}</p></div><div><p className="text-xs font-black text-[var(--admin-primary)]">المطلوب</p><p className="mt-1 text-sm font-bold">انصراف: {row.proposedClockedOutAt ? new Date(row.proposedClockedOutAt).toLocaleString('ar-EG') : 'بدون تغيير'}</p></div></div>{(row.state === 'PendingManager' || row.state === 'PendingHr') && <div className="mt-4 flex gap-2"><button onClick={() => void decide(row, true)} className="admin-btn-primary inline-flex min-h-11 items-center gap-2"><Check className="h-4 w-4" />اعتماد</button><button onClick={() => void decide(row, false)} className="admin-btn-secondary inline-flex min-h-11 items-center gap-2"><X className="h-4 w-4" />رفض</button></div>}</article>; })}</div>;
}
