...
def main() -> int:
    ...
    if args.dry_run:
        print(json.dumps({"release": args.release, "cleanAuditDatabase": True, "productionMigration": True, "status": "dry-run"}))
        return 0
    if not args.yes:
...
"""
    ...
    if completed.returncode:
        ...
        raise MigrationError(
            f"clean audit or production migration gate failed: {detail}"
        )
    print(json.dumps({"release": args.release, "status": "success", "node": node.id}))
    return 0
