...
public class PublicController : ControllerBase
{
    ...
    public async Task<IActionResult> DownloadResource(Guid resourceId, [FromQuery] string token, CancellationToken ct)
    {
        ...
        var rootPath = usesProtectedStorage
            ? Path.Combine(_env.ContentRootPath, "App_Data", "protected", "resources")
            : _env.WebRootPath ?? Path.Combine(Directory.GetCurrentDirectory(), "wwwroot");
        var storageRelativePath = usesProtectedStorage
            ? relativePath[protectedPrefix.Length..]
            : relativePath;

        var physicalPath = Path.Combine(rootPath, storageRelativePath);
        if (!IsUnderRoot(rootPath, physicalPath))
        {
            return BadRequest(new { Success = false, Message = "Invalid resource path." });
        }
        ...
        else
        {
            ...
            return PhysicalFile(physicalPath, contentType, Path.GetFileName(storageRelativePath), enableRangeProcessing: true);
        }
    }

    private static bool IsUnderRoot(string rootPath, string physicalPath)
    {
