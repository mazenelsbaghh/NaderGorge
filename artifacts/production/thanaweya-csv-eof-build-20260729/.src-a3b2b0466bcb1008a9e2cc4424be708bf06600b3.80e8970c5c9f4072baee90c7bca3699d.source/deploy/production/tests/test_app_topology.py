...
def test_signalr_hubs_keep_negotiate_and_transport_on_the_same_node() -> None:
    """Regression: SignalR negotiation failed when round-robin changed nodes."""
    config = (ROOT / "deploy/production/config/haproxy/postgres.cfg").read_text()
    ...
    assert "cookie MASSAR_SIGNALR_NODE insert indirect nocache httponly secure" in signalr_backend
    assert signalr_backend.count("cookie node-") == 3
