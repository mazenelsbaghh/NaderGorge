...
public sealed class HrAuthorizationService : IHrAuthorizationService
{
    ...
    public async Task EnsureAuthorizedAsync(IHrAuthorizedRequest request, CancellationToken ct)
    {
        ...
        // Attendance is available to every provisioned employee.  Older Staff
        // roles may not include the legacy explicit claim, but the employee
        // profile is the authoritative signal that self-attendance applies.
        if (string.Equals(request.RequiredPermission, HrPermissions.AttendanceSelf, StringComparison.OrdinalIgnoreCase)
            && request.RequiredScope == HrAccessScope.Self
            && (!request.ResourceUserId.HasValue || request.ResourceUserId.Value == actorUserId)
            && await _db.EmployeeProfiles.AsNoTracking().AnyAsync(item => item.UserId == actorUserId
                && (!request.ResourceEmployeeId.HasValue || item.Id == request.ResourceEmployeeId.Value), ct))
            return;
