...
def root_owned_builder(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(builder, "redis_role", lambda: "replica")
...
@pytest.mark.parametrize(
    ("patroni", "redis", "reason"),
    [("leader", "replica", "PostgreSQL leader"), ("replica", "master", "Redis master")],
)
def test_remote_builder_refuses_data_service_leaders(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    patroni: str,
    redis: str,
    ...
    target = workspace(monkeypatch, tmp_path)
    with pytest.raises(builder.RemoteBuilderError, match=reason):
        builder.preflight(
            workspace=target,
            release_id=RELEASE,
            expected_source_sha256=builder.source_digest(target / "source"),
            patroni_role_reader=lambda: patroni,
            redis_role_reader=lambda: redis,
        )


def test_remote_builder_fails_closed_when_redis_role_cannot_be_read(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    target = workspace(monkeypatch, tmp_path)
    with pytest.raises(builder.RemoteBuilderError, match="cannot safely determine local Redis role"):
        builder.preflight(
            workspace=target,
            release_id=RELEASE,
            expected_source_sha256=builder.source_digest(target / "source"),
            patroni_role_reader=lambda: "replica",
            redis_role_reader=lambda: (_ for _ in ()).throw(builder.RemoteBuilderError("cannot safely determine local Redis role")),
        )


def test_redis_role_uses_local_secret_only_in_environment_and_never_in_error(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    password_file = tmp_path / "redis-password"
    secret = "never-log-this-redis-secret"
    password_file.write_text(secret + "\n")
    monkeypatch.setattr(builder, "REDIS_PASSWORD_FILE", password_file)
    ...
    assert REDIS_ROLE() == "replica"
    assert secret not in " ".join(observed["argv"])
    assert observed["env"]["REDISCLI_AUTH"] == secret
    ...
    with pytest.raises(builder.RemoteBuilderError) as error:
        REDIS_ROLE()
