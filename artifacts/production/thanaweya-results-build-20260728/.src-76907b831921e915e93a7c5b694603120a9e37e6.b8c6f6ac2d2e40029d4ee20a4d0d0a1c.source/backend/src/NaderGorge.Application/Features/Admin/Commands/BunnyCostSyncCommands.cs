...
public sealed class SyncBunnyUsageCommandHandler : IRequestHandler<SyncBunnyUsageCommand, ApiResponse<BunnyUsageSyncResultDto>>
{
    ...
    public async Task<ApiResponse<BunnyUsageSyncResultDto>> Handle(SyncBunnyUsageCommand request, CancellationToken cancellationToken)
    {
        ...
        foreach (var asset in assets)
        {
            ...
            NaderGorge.Application.Interfaces.BunnyStreamVideoDto? bunnyVideo;
            NaderGorge.Application.Interfaces.BunnyVideoStorageDto? storageInfo;
            try
            {
                ...
                storageInfo = await _bunny.GetVideoStorageAsync(asset.BunnyVideoGuid, cancellationToken);
            }
            catch when (snapshot is not null)
            {
                // A refresh must never replace a previously useful monthly snapshot with zeroes
                // merely because Bunny is temporarily unavailable.
                continue;
            }

            // A missing video response has no trustworthy per-video bandwidth. Retain an existing
            // snapshot rather than silently turning its previous estimate into an unavailable value.
