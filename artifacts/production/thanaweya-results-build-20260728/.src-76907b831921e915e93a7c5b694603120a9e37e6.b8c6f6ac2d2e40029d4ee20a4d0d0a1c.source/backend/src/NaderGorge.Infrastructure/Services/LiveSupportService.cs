    ...
    NaderGorge.Application.Features.LiveSupportAI.Interfaces.ILiveSupportAIRegistrationService? aiRegistrationService = null) : ILiveSupportService, ILiveSupportAssignmentCoordinator
{
    ...
    public async Task<IReadOnlyList<LiveSupportStudentSupportHistoryDto>> GetStudentSupportHistoryAsync(Guid staffUserId, bool isAdmin, Guid conversationId, CancellationToken ct)
    {
        ...
        var conversationIds = await _db.LiveSupportConversations.AsNoTracking()
        ...
        var histories = new List<LiveSupportStudentSupportHistoryDto>(conversationIds.Count);
        foreach (var id in conversationIds)
        {
            var item = await _db.LiveSupportConversations.AsNoTracking().SingleAsync(value => value.Id == id, ct);
            var lastMessage = await _db.LiveSupportMessages.AsNoTracking().Where(message => message.ConversationId == id)
                .OrderByDescending(message => message.SentAt)
                .Select(message => new { message.Content, message.SentAt })
                .FirstOrDefaultAsync(ct);
            var lastEvent = await _db.LiveSupportEvents.AsNoTracking().Where(value => value.ConversationId == id)
                .OrderByDescending(value => value.Sequence)
                .Select(value => value.Type.ToString())
                .FirstOrDefaultAsync(ct);
            var activities = await _db.LiveSupportEvents.AsNoTracking().Where(value => value.ConversationId == id && value.Type != LiveSupportEventType.MessageSent)
                .OrderByDescending(value => value.Sequence).Take(50)
                .OrderBy(value => value.Sequence)
                .Select(value => new LiveSupportStudentSupportActivityDto(value.OccurredAt, value.Type.ToString()))
                .ToListAsync(ct);
            var messageCount = await _db.LiveSupportMessages.CountAsync(message => message.ConversationId == id, ct);
            var lastActivityAt = lastMessage?.SentAt ?? item.ClosedAt ?? item.CreatedAt;
            histories.Add(new LiveSupportStudentSupportHistoryDto(id, item.Status, item.Subject, item.CreatedAt, item.ClosedAt, lastActivityAt, messageCount, TruncateHistoryPreview(lastMessage?.Content), lastEvent, activities));
        }
    ...
    public async Task<IReadOnlyList<LiveSupportMessageDto>> GetStudentHistoryMessagesAsync(Guid staffUserId, bool isAdmin, Guid conversationId, Guid historyConversationId, int pageSize, CancellationToken ct)
    {
        ...
        return await _db.LiveSupportMessages.AsNoTracking().Where(message => message.ConversationId == historyConversationId)
            .OrderByDescending(message => message.SentAt).Take(Math.Clamp(pageSize, 1, 100)).OrderBy(message => message.SentAt)
            .Select(message => ToDto(message)).ToListAsync(ct);
    }
