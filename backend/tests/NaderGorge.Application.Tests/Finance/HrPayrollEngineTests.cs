...
public sealed class HrPayrollEngineTests
{
    ...
    public void FormulaValidator_RejectsArbitraryCodeAndAcceptsConstrainedExpressions()
    {
        ...
        Assert.True(PayrollCalculationEngine.IsValidExpression("attendance.early_leave_minutes * rate"));
    }

    [Fact]
    public void Calculation_AppliesEarlyLeaveDeductionFromAttendanceSnapshot()
    {
        var engine = new PayrollCalculationEngine();
        var deduction = new PayComponent { Code = "EARLY", Name = "Early leave", Classification = PayComponentClass.Deduction };
        var rule = new PayrollRule { PayComponent = deduction, PayComponentId = deduction.Id, Name = "Early leave", Expression = "attendance.early_leave_minutes * rate", Rate = 2m };

        var result = engine.Calculate(new PayrollCalculationInput(Guid.NewGuid(), 1000m, 0, 0, 0, EarlyLeaveMinutes: 15), [rule]);

        Assert.Equal(30m, result.Deductions);
        Assert.Equal(-30m, result.Net);
    }
