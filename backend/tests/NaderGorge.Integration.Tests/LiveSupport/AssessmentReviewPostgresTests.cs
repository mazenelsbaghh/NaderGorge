...
public sealed class AssessmentReviewPostgresTests
{
    ...
    [InlineData(20, 1, 20)]
    [InlineData(20, 0, 0)]
    public async Task HomeworkAICompletesPendingAttemptsAndDuplicateCallbacksDoNotChangeGrades(int total, int correctness, int expected)
    {
        await using var fixture = new PostgresLiveSupportFixture();
        await fixture.ResetAsync();
        var seed = await Seed(fixture.Db);
        seed.Homework.TotalScore = total;
        seed.Homework.PassingScoreThreshold = total == 0 ? 0 : 10;
        seed.Submission.DefinitionSnapshotJson = AssessmentDefinitionSnapshot.FromHomework(seed.Homework).ToJson();
        await fixture.Db.SaveChangesAsync();
        var previousLesson = await fixture.Db.Lessons.SingleAsync(l => l.Id == seed.Homework.LessonId);
        var nextLesson = new Lesson { Title = "Next lesson", ContentSectionId = previousLesson.ContentSectionId, Order = previousLesson.Order + 1 };
        fixture.Db.Lessons.Add(nextLesson);
        fixture.Db.StudentAccessGrants.Add(new() { UserId = seed.Submission.StudentId, LessonId = nextLesson.Id, GrantType = CodeType.Lesson, IsActive = true });
        await fixture.Db.SaveChangesAsync();
        var lessonReader = new NaderGorge.Application.Features.Content.Queries.GetLessonDetailQueryHandler(fixture.Db,
            new AccessCheckService(fixture.Db), new TeacherAuthorizationService(fixture.Db));
        var before = await lessonReader.Handle(new(nextLesson.Id, seed.Submission.StudentId), default);
        Assert.True(before.Success, before.Message);
        Assert.True(before.Data!.IsLocked);
        var recovery = new HomeworkGradingRecoveryService(fixture.Db);
        Assert.Contains(seed.Submission.Id, await recovery.FindBatchAsync(Guid.Empty, default));
        Assert.True(await recovery.RecoverAsync(seed.Submission.Id, default));
        Assert.False(await recovery.RecoverAsync(seed.Submission.Id, default));
        var queued = await fixture.Db.OutboxEvents.SingleAsync(e => e.Type == HomeworkEvaluationQueue.EventType);
        var payload = System.Text.Json.JsonSerializer.Deserialize<HomeworkEvaluationPayload>(queued.PayloadJson)!;
        var answerId = Assert.Single(payload.Questions).AnswerId;
        var command = new WebhookHomeworkGradedCommand(seed.Submission.Id, payload.Fingerprint, [new(answerId, correctness, "AI explanation")]);
        var handler = new WebhookHomeworkGradedCommandHandler(fixture.Db);
        var graded = await handler.Handle(command, default);
        Assert.True(graded.Success, graded.Message);
        Assert.Equal("Graded", graded.Data!.Status);
        Assert.Equal("Superseded", (await handler.Handle(command, default)).Data!.Status);
        fixture.Db.ChangeTracker.Clear();
        var saved = await fixture.Db.HomeworkSubmissions.Include(s => s.Answers).SingleAsync(s => s.Id == seed.Submission.Id);
        Assert.Equal(expected, saved.OverallScore);
        Assert.Equal(total == 0 ? 4 : total, saved.TotalScoreSnapshot);
        Assert.NotNull(saved.GradedAt);
        var after = await lessonReader.Handle(new(nextLesson.Id, seed.Submission.StudentId), default);
        Assert.True(after.Success, after.Message);
        Assert.Equal(correctness == 0, after.Data!.IsLocked);
        Assert.Equal(1, await fixture.Db.OutboxEvents.CountAsync(e => e.Type == "HomeworkGraded"));
        Assert.Equal(1, await fixture.Db.AuditLogs.CountAsync(e => e.Action == "HomeworkAIGraded" && e.EntityId == saved.Id));
    }

    [Fact]
    public async Task ConcurrentHomeworkAIRecoveryAndCallbacksProduceOneFinalResult()
    {
        await using var fixture = new PostgresLiveSupportFixture();
        await fixture.ResetAsync();
        var seed = await Seed(fixture.Db);
        ...
        var recovered = await Task.WhenAll(Enumerable.Range(0, 2).Select(async _ =>
        {
            ...
            return await new HomeworkGradingRecoveryService(db).RecoverAsync(seed.Submission.Id, default);
        }));
        Assert.Single(recovered, success => success);
        var queued = await fixture.Db.OutboxEvents.SingleAsync(e => e.Type == HomeworkEvaluationQueue.EventType);
        var payload = System.Text.Json.JsonSerializer.Deserialize<HomeworkEvaluationPayload>(queued.PayloadJson)!;
        var callbacks = await Task.WhenAll(Enumerable.Range(0, 2).Select(async _ =>
        {
            await using var db = new AppDbContext(new DbContextOptionsBuilder<AppDbContext>().UseNpgsql(fixture.ConnectionString).Options);
            return await new WebhookHomeworkGradedCommandHandler(db).Handle(new(seed.Submission.Id, payload.Fingerprint,
                [new(payload.Questions.Single().AnswerId, 1, "correct")]), default);
        }));
        Assert.All(callbacks, receipt => Assert.True(receipt.Success, receipt.Message));
        Assert.Single(callbacks, receipt => receipt.Data!.Status == "Graded");
        Assert.Equal(1, await fixture.Db.OutboxEvents.CountAsync(e => e.Type == "HomeworkGraded"));
    }
    ...
    [InlineData("changed-definition")]
    [InlineData("manual-grade")]
    public async Task HomeworkAIRejectsInvalidResultsAndCannotOverwriteChangedOrManuallyGradedAttempts(string scenario)
    {
        await using var fixture = new PostgresLiveSupportFixture();
        await fixture.ResetAsync();
        var seed = await Seed(fixture.Db);
        ...
        var answerId = seed.Submission.Answers.Single().Id;
        HomeworkAIGrade[] grades = [new(answerId, 1, "feedback")];
        if (scenario == "wrong-answer") grades = [new(Guid.NewGuid(), 1, "feedback")];
        if (scenario == "invalid-score") grades = [new(answerId, 2, "feedback")];
        if (scenario == "duplicate") grades = [grades[0], grades[0]];
        if (scenario == "changed-definition")
            seed.Submission.DefinitionSnapshotJson = (AssessmentDefinitionSnapshot.Read(seed.Submission.DefinitionSnapshotJson!, "homework", seed.Homework.Id)
                with { RevisionId = Guid.NewGuid() }).ToJson();
        if (scenario == "manual-grade") { seed.Submission.Status = SubmissionStatus.Graded; seed.Submission.OverallScore = 7; }
        await fixture.Db.SaveChangesAsync();
        var response = await new WebhookHomeworkGradedCommandHandler(fixture.Db).Handle(new(seed.Submission.Id, fingerprint, grades), default);
        Assert.Equal(scenario is "changed-definition" or "manual-grade", response.Success);
        fixture.Db.ChangeTracker.Clear();
        var saved = await fixture.Db.HomeworkSubmissions.SingleAsync(s => s.Id == seed.Submission.Id);
        Assert.Equal(scenario == "manual-grade" ? 7 : 0, saved.OverallScore);
        Assert.False(await fixture.Db.OutboxEvents.AnyAsync(e => e.Type == "HomeworkGraded"));
    }
