...
public sealed class HrAttendanceController : ControllerBase
{
    ...
    public async Task<IActionResult> Today(CancellationToken ct)
    {
        ...
        var session = await _db.AttendanceSessions.AsNoTracking().Where(item => item.Employee!.UserId == userId)
            .OrderByDescending(item => item.ClockedInAt).Select(item => new
            {
                ...
                breaks = item.Breaks.OrderBy(row => row.StartedAt).Select(row => new { row.Id, row.StartedAt, row.EndedAt })
    ...
    [HasPermission(HrPermissions.AttendanceSelf)]
    public async Task<IActionResult> StartBreak([FromHeader(Name = "Idempotency-Key")] string? key, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(key)) return BadRequest();
        var result = await _mediator.Send(new StartAttendanceBreakCommand(User.RequireUserId(), key, DateTime.UtcNow), ct);
    ...
    public async Task<IActionResult> Sessions(DateOnly? from, DateOnly? to, CancellationToken ct) => Ok(await _db.AttendanceSessions.AsNoTracking()
        ...
        .OrderByDescending(item => item.WorkDate).Select(item => new { item.Id, item.EmployeeId, employee = item.Employee!.User!.FullName, item.WorkDate, item.ClockedInAt, item.ClockedOutAt, state = item.State.ToString(), item.WorkedMinutes }).Take(100).ToListAsync(ct));
