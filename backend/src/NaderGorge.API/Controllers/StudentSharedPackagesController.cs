...
public class StudentSharedPackagesController : ControllerBase
{
    ...
    public async Task<IActionResult> Purchase([FromRoute] Guid id, [FromBody] PurchaseSharedPackageDto? dto, CancellationToken ct)
    {
        ...
        var allocations = selectedTeachers.Select(teacher =>
        {
            var basisAmount = selectedItems
                .Where(item => item.TeacherId == teacher.TeacherId && item.SubjectId == teacher.SubjectId)
                .Sum(item => item.Price);
            var teacherShare = teacher.AllocationMode == TeacherAllocationMode.FixedAmount
                ? teacher.AllocationValue
                : Math.Round(basisAmount * teacher.AllocationValue / 100m, 2, MidpointRounding.AwayFromZero);
            return new TeacherFinancialAllocationInput(
                teacher.TeacherId,
        ...
        var teacherTotal = allocations.Sum(x => x.TeacherShareAmount);
        var platformShare = Math.Max(0m, package.Price - teacherTotal);
        allocations = allocations
            .Select((x, index) => x with { PlatformShareAmount = index == 0 ? platformShare : 0m })
            .ToList();
...
public record PurchaseSharedPackageDto(List<SharedPackageTeacherSelectionDto> Selections);
