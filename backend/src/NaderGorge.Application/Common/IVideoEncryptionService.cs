...
public interface IVideoEncryptionService
{
    ...
    /// </summary>
    string EncryptVideoInfo(string providerName, string providerVideoId, string sessionKey);

    /// <summary>
    /// Decrypts the session token to retrieve the provider video ID (mostly for tests/validation).
    /// </summary>
    (string ProviderName, string ProviderVideoId) DecryptVideoInfo(string encryptedToken, string sessionKey);
