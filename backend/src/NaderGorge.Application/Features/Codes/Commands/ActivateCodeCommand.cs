...
public class ActivateCodeCommandHandler : IRequestHandler<ActivateCodeCommand, ApiResponse<ActivateCodeResponse>>
{
    ...
    public async Task<ApiResponse<ActivateCodeResponse>> Handle(ActivateCodeCommand request, CancellationToken ct)
    {
        try
        {
            ...
            if (codeType != CodeType.Balance && codeGroup.AccountingTiming != CodeAccountingTiming.Immediate)
            {
                decimal itemPrice = 0;
                switch (codeType)
                {
                ...
                var (teacherShare, platformShare, allocationMode, allocationValue, basisAmount) =
                    BulkGenerateCodesCommandHandler.CalculateShares(
                        finalPrice,
                        teacherProfile,
                        codeGroup.RevenueOwner,
                        codeGroup.RevenueAllocationMode,
                        codeGroup.RevenueAllocationValue);
