...
public record CreateEmployeeCommand(
    ...
    DateOnly? ShiftEffectiveFrom = null) : IRequest<ApiResponse<CreateEmployeeResult>>, IHrAuthorizedRequest
{
...
public sealed class CreateEmployeeCommandValidator : AbstractValidator<CreateEmployeeCommand>
{
    public CreateEmployeeCommandValidator()
    {
        ...
        RuleFor(item => item.ShiftEffectiveFrom)
            .NotNull()
            .When(item => item.ShiftTemplateId.HasValue)
            .WithMessage("Shift effective date is required when a shift is selected");
    }
}
    ...
    : IRequestHandler<CreateEmployeeCommand, ApiResponse<CreateEmployeeResult>>
{
    ...
    public async Task<ApiResponse<CreateEmployeeResult>> Handle(CreateEmployeeCommand request, CancellationToken ct)
    {
        ...
        {
            shiftTemplate = await _db.ShiftTemplates
                .Include(item => item.WorkCalendar)
                .SingleOrDefaultAsync(
                    item => item.Id == request.ShiftTemplateId.Value && item.IsActive,
                    ct);
            if (shiftTemplate is null || shiftTemplate.WorkCalendar is null || !shiftTemplate.WorkCalendar.IsActive)
            {
                return ApiResponse<CreateEmployeeResult>.Fail(
                    "الشفت المحدد غير صالح أو غير نشط",
                    new List<string> { "SHIFT_INVALID" });
            }
        }
        ...
        await _audit.WriteMutationAsync("CreateEmployee", nameof(EmployeeProfile), profile.Id, null, new
        {
            ...
            shiftTemplateId = shiftTemplate?.Id,
            shiftEffectiveFrom = request.ShiftEffectiveFrom
