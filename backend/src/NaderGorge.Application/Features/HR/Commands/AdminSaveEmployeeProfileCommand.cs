...
public record AdminSaveEmployeeProfileCommand(
    ...
    int DailyBreakAllowanceMinutes = 30,
    int ShortPermissionMaxMinutes = 5,
    int DailyShortPermissionAllowanceMinutes = 15
...
public class AdminSaveEmployeeProfileCommandValidator : AbstractValidator<AdminSaveEmployeeProfileCommand>
{
    public AdminSaveEmployeeProfileCommandValidator()
    {
        ...
        RuleFor(x => x.DailyBreakAllowanceMinutes).InclusiveBetween(0, 240);
        RuleFor(x => x.ShortPermissionMaxMinutes).InclusiveBetween(0, 60);
        RuleFor(x => x.DailyShortPermissionAllowanceMinutes).InclusiveBetween(0, 240);
    ...
    public async Task<ApiResponse<EmployeeProfileMutationResult>> Handle(AdminSaveEmployeeProfileCommand request, CancellationToken ct)
    {
        ...
        if (isNew)
        {
            var profile = new EmployeeProfile
            {
                ...
                ,DailyBreakAllowanceMinutes = request.DailyBreakAllowanceMinutes
                ,ShortPermissionMaxMinutes = request.ShortPermissionMaxMinutes
                ,DailyShortPermissionAllowanceMinutes = request.DailyShortPermissionAllowanceMinutes
            };
        ...
        else
        {
            ...
            user.EmployeeProfile.DailyBreakAllowanceMinutes = request.DailyBreakAllowanceMinutes;
            user.EmployeeProfile.ShortPermissionMaxMinutes = request.ShortPermissionMaxMinutes;
            user.EmployeeProfile.DailyShortPermissionAllowanceMinutes = request.DailyShortPermissionAllowanceMinutes;
