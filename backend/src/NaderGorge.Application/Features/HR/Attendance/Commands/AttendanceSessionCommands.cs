...
public sealed class ClockInAttendanceCommandHandler : IRequestHandler<ClockInAttendanceCommand, ApiResponse<AttendanceMutationResult>>
{
    ...
    public async Task<ApiResponse<AttendanceMutationResult>> Handle(ClockInAttendanceCommand request, CancellationToken ct)
    {
        ...
        var segment = shift.ShiftTemplate!.Segments.OrderBy(item => item.Sequence).First();
...
public sealed record StartAttendanceBreakCommand(Guid UserId, string IdempotencyKey, DateTime OccurredAt)
...
public sealed class StartAttendanceBreakCommandHandler : IRequestHandler<StartAttendanceBreakCommand, ApiResponse<Guid>>
{
    ...
    public StartAttendanceBreakCommandHandler(IAppDbContext db, IHrAuditWriter? audit = null) { _db = db; _audit = audit ?? new HrAuditWriter(db, DetachedHrRequestContext.Instance); }
    public async Task<ApiResponse<Guid>> Handle(StartAttendanceBreakCommand request, CancellationToken ct)
    {
        ...
        var occurredAt = EnsureUtc(request.OccurredAt); var attendanceBreak = new AttendanceBreak { AttendanceSessionId = session.Id, StartedAt = occurredAt };
        ...
        await _audit.WriteMutationAsync("AttendanceBreakStart", nameof(AttendanceBreak), attendanceBreak.Id, null, new { attendanceBreak.StartedAt }, "Employee break start", ct, request.UserId);
        await _db.SaveChangesAsync(ct); return ApiResponse<Guid>.Ok(attendanceBreak.Id);
    }
    private static DateTime EnsureUtc(DateTime value) => value.Kind == DateTimeKind.Utc ? value : value.ToUniversalTime();
}
...
{
    private readonly IAppDbContext _db; private readonly IHrAuditWriter _audit;
    public EndAttendanceBreakCommandHandler(IAppDbContext db, IHrAuditWriter? audit = null) { _db = db; _audit = audit ?? new HrAuditWriter(db, DetachedHrRequestContext.Instance); }
