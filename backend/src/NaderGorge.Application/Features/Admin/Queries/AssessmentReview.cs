    ...
    : IRequestHandler<GetAssessmentReviewQuery, ApiResponse<AssessmentReviewDto>>
{
    ...
    private async Task<AssessmentReviewDto?> HomeworkReview(AssessmentTarget target, CancellationToken ct)
    {
        ...
        var questions = submission.Homework.Questions.OrderBy(q => q.Order).Select(q =>
        {
        ...
        return new(submission.Id, submission.Student.FullName, submission.Homework.Title, submission.OverallScore,
            submission.Homework.TotalScore, submission.Status.ToString(), submission.SubmittedAt != null, submission.AssistantNotes, questions);
    }
    ...
    private async Task<AssessmentReviewDto?> ExamReview(AssessmentTarget target, CancellationToken ct)
    {
        ...
        var questions = attempt.Exam.ExamQuestions.Where(q => assignedIds.Contains(q.Id)).OrderBy(q => q.Order).Select(q =>
        {
            ...
            return new AssessmentAnswerReview(q.Id, q.Order, q.Question.Text, q.Question.ImageUrl,
                essay?.AnswerText ?? answer?.SelectedOption?.Text ?? answer?.SubmittedText, essay?.AudioUrl,
        ...
        return new(attempt.Id, attempt.User.FullName, attempt.Exam.Title, attempt.ScoreAchieved, attempt.Exam.TotalScore,
