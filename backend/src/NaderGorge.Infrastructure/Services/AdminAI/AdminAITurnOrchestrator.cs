...
public sealed class AdminAITurnOrchestrator(IAppDbContext db, IAdminAIAccessGate access, IAdminAIDataProtector protector) : IAdminAITurnOrchestrator
{
    public async Task<AdminAITurnDto> QueueAsync(Guid actorId, Guid conversationId, string content, long expectedVersion, string idempotencyKey, CancellationToken ct)
    {
        ...
        if (await db.AdminAITurns.AnyAsync(x => x.ConversationId == conversationId && x.Status != AdminAITurnStatus.Completed && x.Status != AdminAITurnStatus.Cancelled && x.Status != AdminAITurnStatus.Failed && x.Status != AdminAITurnStatus.AccessRevoked, ct)) throw new AdminAIConflictException(AdminAIErrorCodes.ActiveTurnExists);
        if (await db.AdminAITurns.CountAsync(x => x.ActorAdminUserId == actorId && x.Status != AdminAITurnStatus.Completed && x.Status != AdminAITurnStatus.Cancelled && x.Status != AdminAITurnStatus.Failed && x.Status != AdminAITurnStatus.AccessRevoked, ct) >= 2) throw new AdminAIConflictException(AdminAIErrorCodes.ActiveTurnLimit);
